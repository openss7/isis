...
join_init()
{
	isis_entry(GENERIC_JOIN_REQ, (vfunc *) pg_join_req, "cl_join:pg_join_req");
	isis_entry(GENERIC_CLIENT_REQ, (vfunc *) pg_client_req, "cl_join:pg_client_req");
	isis_entry(GENERIC_XFER_STATE, (vfunc *) xfer_rcv_small, "cl_join:xfer_rcv_small");
	...
	va_list *ap;
{
	...
	if (code)
		...
		while (code);
	if (logged) {
		if (log_init(logfname, gname, lentry, flush_when, log_routine, coldstart, ignoreold)
		    == NULL)
			goto err;
		if (nmbuf)
	...
	int how;
{
	...
	if (gip->gi_join_verifier == 0 ||
		...
		xfer_in_progress = 1;
		if ((wid = pg_watch(gaddr, &who, W_JOIN, (vfunc *) xfer_send, (void *) &xi)) == 0)
			/* Already a member */
			...
			if (xfer_in_progress)
				t_wait_l(&xfer_done,
					 "isis system: coordinator waiting for xfer_send to finish");
		}
	} else if (xi.x_rval == -1)
	...
	xinfo *xi;
{
	...
	if (xi->x_how == TAKEOVER) {
		...
		if (fbcast
		    (&xi->x_who, GENERIC_XFER_WHERE, "", 1, "%d,%d", &where.domain,
		     &where.locator) != 1)
			xi->x_rval = IE_BROKEN;
	} else {
		where.domain = 0;
	...
	if (xfermp_count > 0)
		(void) fbcast(&xi->x_who, GENERIC_XFER_WHERE, "", 1, "%d,%d", &where.domain,
			      &where.locator);
	xfer_in_progress = 0;
	t_sig(&xfer_done, 0);
}
	...
	char *credentials;
{
	...
	ISIS_ENTER();
	if ((rval =
	     bcast(gaddr, GENERIC_CLIENT_REQ, "%A[1]/%s", gaddr, credentials, 1, "%d",
		   &err)) == 1) {
		if ((isis_errno = err) == 0)
			ISIS_RETURN(0);
		ISIS_RETURN(-1);
	...
	register message *mp;
{
	...
	if (msg_get(mp, "%a", &gaddr) != 1)
		panic("pg_client_req");
	coord_cohort(mp, &gaddr, (vfunc *) pg_client_action, NULLROUTINE, NULLARG);
}
	...
	VOID *arg;
{
	...
	else if (gip->gi_client_verifier && _ISISCALL1(gip->gi_client_verifier, credentials) == -1)
