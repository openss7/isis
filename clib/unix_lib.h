...
extern void bcopy(VOID * from, VOID * to, int n_bytes);
		...
		/* Isis defines VOID as char on some compiler that give compile errors on void * in 
		   assignments. */
...
extern int select(int width, int *readfds, int *writefds, int *exceptfds, struct timeval *timeout);
