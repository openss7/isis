...
static
    void
pg_detect_pfailure(gaddrp, event, wp)
	address *gaddrp;		/* May be the group address if called via pg_watching of a
					   member process address if called via proc_watch. */
	int event;
	register wnode *wp;
{
	address gaddr;
	...
	VOID *arg;
{
	...
	if (am_client == 0) {
		wp->w_jid = proc_watch(gv->gv_members, (vfunc *) pg_detect_pfailure, wp);
		fbcast(gv->gv_members, GENERIC_WATCHING, "%d,%A[1],%d", POST_WATCH, gaddrp,
		       wp->w_id, 0);
	} else
		wp->w_jid = 0;
	act_ev(1, wp->w_act, ACT_WATCH);
	...
	register message *mp;
{
	...
	if (req == POST_WATCH) {
		if (gip == 0 || pg_rank(&gaddr, &my_address) < 0) {
			fbcast(msg_getsender(mp), GENERIC_WATCHING, "%d,%A[1],%d", TRIGGER, gaddr,
			       wid, 0);
			return;
		}
	...
	register qnode *qp;
{
	...
	jid = wp->w_jid;
	if (wp->w_xbyref)
		ISISCALL4(wp->w_proc, &wp->w_gaddr, &qp->qu_pname, &wp->w_mode, &wp->w_arg);
	else
		ISISCALL4(wp->w_proc, &wp->w_gaddr, &qp->qu_pname, wp->w_mode, wp->w_arg);
	qu_free(qp);
	if (jid)
	...
	if (gv->gv_nmemb == 0) {
		...
		if (gip = map_gaddr(&gv->gv_gaddr))
			while (qp = qu_head(gip->gi_watching)) {
				...
				qu_remove(qp);
				fbcast(&qp->qu_pname, GENERIC_WATCHING, "%d,%A[1],%d", TRIGGER,
				       &gv->gv_gaddr, qp->qu_data, 0);
				qu_free(qp);
			}
	}
	...
	register address *ap;
{
	...
	while (isis_joining)
		(void) t_wait_l(&wants_to_run_after_join,
				"isis system: proc_failed waiting for join to finish");
	for (qp = isis_pwlist->qu_next; qp != isis_pwlist; qp = nqp) {
		nqp = qp->qu_next;
		if (act_blocked && qp->qu_wnode->w_act != act_blocked)
			continue;
