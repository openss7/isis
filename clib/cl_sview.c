	...
	register message *mp;
{
	...
	for (qp = isis_swlist->qu_next; qp != isis_swlist; qp = nqp) {
		...
		if (act_blocked && svm->sv_act != act_blocked)
			continue;
		if (svm->sv_viewid == sp->sv_viewid
		    || ((svm->sv_viewid & 0xFF) == (sp->sv_viewid & 0xFF)
	...
	for (qp = isis_tasks->qu_next; qp != isis_tasks; qp = qp->qu_next) {
		...
		if (tp->task_queue == &tp->task_mwant) {
			...
			for (sid = sp->sv_slist; *sid; sid++)
				if (SITE_NO(*sid) == siteno
				    && (SITE_INCARN(*sid) == inc || inc == RECOVERY_INCARN))
					break;
			if (*sid == 0)
...
site_monitor_dump()
{
	...
	for (qp = isis_swlist->qu_next; qp != isis_swlist; qp = qp->qu_next) {
		...
		switch (svm->sv_event) {
		case W_MONITOR:
			print("monitoring site-views, ");
			break;
		case W_FAIL:
			print("watching site %d/%d, ", SITE_NO(svm->sv_sid),
			      SITE_INCARN(svm->sv_sid));
			break;
		case W_RECOVER:
			print("waiting for site %d to recover, ", SITE_NO(svm->sv_sid));
			break;
		default:
			print(" *** contains a sv_watch event code %d, site %d/%d *** ",
			      svm->sv_event, SITE_NO(svm->sv_sid), SITE_INCARN(svm->sv_sid));
			break;
		}
		print("Call ");
