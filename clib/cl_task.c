    ...
    /* Called to deinitialize the ISIS task world when unwinding in lisp. */
{
	...
	/* Probably lots of other things to do... including killing(?) all ISIS tasks, deleting
	   messages etc. */
}
...
call_allegro(routine, arg)
  ...
  /* Hack to get around Allegro register trashing bug. Its slow but * it should be fixed soon. */
{
	struct {
...
invoke()
{
	...
	print("invoke: tp %x obtained MUTEX call %s %x(0x%x)\n",
	      tp, delayed_task ? "delayed" : "urgent", tp->task_routine, tp->task_arg0);
#endif
	new_task = isis_ctp;
	if (delayed_task) {
...
t_yield()
{
	...
	ISIS_ENTER();
	if (isis_ctp != isis_scheduler)
		ISIS_RETURN(t_wait_l
			    (&isis_runqueue, "isis system: wants to let other tasks run first"));
	tp = task_dequeue(&isis_runqueue);
	if (tp != isis_ctp)
		task_swtch(tp, NULLROUTINE, NULLARG);
	ISIS_RETURN((void *) isis_ctp->task_rval);
	...
	for (qp = isis_tasks->qu_next; qp != isis_tasks; qp = qp->qu_next) {
		...
		if (tp->task_threadid == self) {
			...
			if (cp && (cp != &isis_runqueue || isis_scheduler || isis_ctp != DUMMYTASK)) {
				...
				if (isis_scheduler == 0)
					db_print
					    ("there isn't any scheduler so thread_isis_enter is starting one\n",
					     0);
				else
					db_print
					    ("there is already a scheduler so thread_isis_enter is just going to wait\n",
					     0);
