	...
	int a0, a1, a2, a3;
{
	...
	if (my_process_id < -10)
		print("\nCLIENT PROCESS remote_%d INTERNAL DUMP REQUESTED: ",
		      my_process_id & (short) ~PID_REMOTE);
	else
...
#           endif
	...
	if (level & DUMP_MEM) {
		print("Memory mgt: %d allocs, %d frees, %d bytes in use\n", nalloc, nfree,
		      memalloc - memfree);
		print
		    ("Message counts: %d allocs %d frees (%d in use), %d enqueued by MSG_ENQUEUE\n",
		     msg_namsgs, msg_nfmsgs, msg_namsgs - msg_nfmsgs, isis_msgs_enqueued);
		if (isis_ignored_replies)
			print
			    ("*** Ignored %d replies, %d were nullreplies (no bcast-task was waiting)\n",
	...
	address *addr;
{
	...
	case ISAGID:
		print("gaddr=%d/%d.%d[", addr->addr_site, addr->addr_incarn, addr->addr_groupid);
		pentry(addr, addr->addr_entry);
		print("])");
		return;
	case ISALGID:
		print("lgaddr=%d/%d.%d[", addr->addr_site, addr->addr_incarn, addr->addr_groupid);
		pentry(addr, addr->addr_entry);
		print("])");
		return;
	case ISPLIST:
		print("plist=%d/%d:%d)", addr->addr_process, addr->addr_site, addr->addr_incarn);
		return;
	case ISACT:
		print("act=");
	default:
...
#              define sparc_sp (tp-> task_env[1])
	if (isis_ctp != tp)
		print(", pc=0x%x sp=0x%x", tp->task_env[0], sparc_sp);
#          endif
#       endif
	print("]: ");
		...
		if (tp->task_sleep) {
			...
			if (tp->task_dests) {
				print("\n   Wants %d of %d replies, got %d+%d null, msgid=%d\n",
				      tp->task_nwant, tp->task_nsent, tp->task_nreplies,
				      tp->task_nullreps, tp->task_msgid);
				print("   Dests: ");
				paddrs(tp->task_dests);
				print("; stat <%s>", tp->task_done);
