...
bc_dump()
{
	...
	for (qp = bc_list->qu_next; qp != bc_list; qp = qp->qu_next) {
		...
		print("bc %x: gip %x ndests %d, seqn %d, ", bc, bc->bc_gip, bc->bc_ndests,
		      bc->bc_seqn);
...
#	if (BYP_VERBOSE)
	bseqn = byi->by_bseq;
	print("[%d]: got [%d %d %d %d] in ", my_process_id, bseqn[0], bseqn[1], bseqn[2], bseqn[3]);
	paddr(&byi->by_group);
...
#                           if (BYP_VERBOSE)
						print
						    ("... still delayed (%d > %d for rank %d -- mine %d)\n",
						     *--bseqn, *--gp, n, myrank);
#                           endif
						break;
					}
...
#			    endif
						...
						msg_increfcount(mp);
						pg_add_mp(gip->gi_aborder, gip->gi_gaddr, mp,
							  MSG_DELETE);
#			    if (BYP_VERBOSE)
...
bypass_sendterminate()
{
	...
	while (qp = qu_head(term_queue)) {
		...
		*ap = NULLADDRESS;
		...
		byi->by_myseqn = gip->gi_myseqn;
		n = gip->gi_transport;
		while ((*by_transport[n])
		       (gip->gi_gaddr, TRUE, mp, dests, NULLROUTINE, NULLARG, NULLARG) == -1 && n)
			n = 0;
		msg_delete(mp);
	}
	force_lazy(FALSE);
	...
	register message *mp;
{
	...
	if (entry != GENERIC_NEW_VIEW)
		return;
	if ((npg =
	     (sys_groupview *) msg_getfield(mp, CL_NEWVIEW, 1, (int *) 0)) == (sys_groupview *) 0)
		return;
	...
	if (!addr_isnull(&bv->gv_departed)) {
		...
		for (qp = isis_tasks->qu_next; qp != isis_tasks; qp = qp->qu_next) {
			...
			if (tp->task_queue == &tp->task_mwant
			    && tp->task_sentto.addr_process != PROTOCOLS) {
				if (failed)
...
#			if (BYP_VERBOSE)
					...
					bseqn = byi->by_bseq;
					print
					    ("[%d]: send copy of [%d %d %d %d] from %d view %d.%d\n",
					     my_process_id, bseqn[0], bseqn[1], bseqn[2], bseqn[3],
					     msg_getsender(msg)->addr_process, VMM(byi->by_viewid));
#			endif
					/* 
					...
					dlist = dests;
					while (!aptr_isnull(ap)) {
						if (!addr_isequal(ap, msg_getsender(msg))
						    && !addr_ismine(ap))
							*dlist++ = *ap;
						ap++;
					}
					if (dlist != dests) {
						*dlist = NULLADDRESS;
						(*by_transport[gip->gi_transport]) (gip->gi_gaddr,
										    TRUE, msg,
										    dests,
										    NULLROUTINE,
										    NULLARG,
	...
	if (myseqn != -1) {
		mp = msg_gen("%A[1],%A[1],%d,%d", gip->gi_gaddr, &my_address, viewid, myseqn);
...
#           endif
	...
	if (gip->gi_flushcount < gip->gi_flushwant) {
		if ((int) t_wait_l(&gip->gi_bypasswait, "bypass_checkview:do flush") == -1)
			panic("left group during flush protocol");
	...
	*dp = 0;
	...
	while (nreplies < nwant && nsent > 0) {
		...
		if (ap || msg_getfield(mp, FLD_ISNULLREP, 1, (int *) 0)) {
			...
			if (sv->sv_incarn[fwd[FWI_NEWDEST].addr_site] ==
			    fwd[FWI_NEWDEST].addr_incarn) {
...
by_dump()
{
	...
	if (gip) {
		for (gip = isis_groups; gip; gip = gip->gi_next) {
			...
			paddr(gip->gi_gaddr);
			if (gip->gi_activebsnd + gip->gi_activemsg + gip->gi_activebrcv)
				print("<< active: bsnd %d msg %d brcv %d >>", gip->gi_activebsnd,
				      gip->gi_activemsg, gip->gi_activebrcv);
			print("\n");
			...
			for (qp = gip->gi_bypassq->qu_next; qp != gip->gi_bypassq; qp = qp->qu_next) {
				...
				print("  From ");
				paddr(&gip->gi_bypassview->gv_members[qp->qu_name]);
				byi = msg_getbyi(mp);
				b = byi->by_bseq;
			...
			for (qp = gip->gi_byprocs->qu_next; qp != gip->gi_byprocs; qp = qp->qu_next) {
				...
				for (np = qp->qu_queue->qu_next; np != qp->qu_queue;
				     np = np->qu_next)
					print
					    ("  Retained copy of msg seqn=%d, flag=%d, mp=%x view %d.%d\n",
					     np->qu_mseqn, np->qu_flag, np->qu_msg,
					     VMM(msg_getbyi(np->qu_msg)->by_viewid));
			}
