	...
	if ((bc->bc_options & (ISISBC_TASK | ISISBC_TIMEOUT)) == 0) {
		...
		*ap = (va_list) ((*(char **) ap) + off);
		...
		if (bc->bc_options & ISISBC_TIMEOUT) {
			...
			bc->bc_rval = 0;
			tid =
			    isis_timeout(bc->bc_timeout * 1000, (vfunc *) t_sig,
					 (VOID *) & bc->bc_await, NULLARG);
			rval = bc_wait(rval);
	...
	register bcid;
{
	...
	if (bc_queue == 0 || (qp = qu_find(bc_queue, bcid)) == NULLQP)
		ISIS_RETURN(0);
	BEGINFROMC
	    gbcast(&qp->qu_bc->bc_dest, GENERIC_G_CANCEL, "%E[1]", bc_getevent(bcid), 1, "%d",
		   &outcome);
	ENDFROMC ISIS_RETURN(outcome);
}
	...
	register bc_args *bc;
{
	...
	if (bc->bc_options) {
		...
		if (bc->bc_options & ISISBC_EXCLUDE)
			msg_replacefield(msg, SYSFLD_EXCLUDE, (char *) &my_address, FTYPE_ADDRESS,
					 sizeof(address));
		...
		begin {
			...
			if (flag) {
				if (alen == 2)
					...
					msg_replacefield(msg, SYSFLD_ALIST, (char *) alist,
							 FTYPE_ADDRESS, sizeof(address) * alen);
			}
		}
	} else {
	...
	if ((bc->bc_pname != CL_GBCAST && bc->bc_pname != CL_GBCAST_GROW) &&
		...
		if (bc->bc_options & ISISBC_BYPASS)
			print
			    ("WARNING: bcast option B was specified but can't use BYPASS protocol!\n");
	...
	va_list *ap;
{
	...
	if (msgid = msg_getid(msg)) {
		...
		if (msg_doputf(rmsg, SYSFLD_SCAN, ap) == -1)
			panic("REPLY: CAN'T GENERATE!");
		msg_setid(rmsg, msgid);
		if (isis_ctp->task_cohorts == 0) {
			BEGINFROMC cbcast_l("mR", msg_getreplyto(msg), GENERIC_RCV_REPLY, rmsg, 0);
		ENDFROMC} else {
			msg_insertfield(rmsg, FLD_TRUESENDER, (char *) &isis_ctp->task_truesender,
					FTYPE_ADDRESS, sizeof(address));
			BEGINFROMC cbcast_l("mlR", isis_ctp->task_cohorts, rmsg, 0);
		ENDFROMC}
		msg_delete(rmsg);
		use_transport = 0;
	}
	...
	va_list *ap;
{
	...
	while (*opstr)
		switch (*opstr++) {
		default:
			panic("reply_l: bad option");
		case 'm':
			options |= ISISBC_MSEND;
			continue;
		case 'c':
			options |= ISISBC_CC;
			continue;
	...
	*rp = NULLADDRESS;
	...
	if (msgid = msg_getid(msg)) {
		...
		if (isis_ctp->task_cohorts)
			msg_insertfield(msg, FLD_TRUESENDER, (char *) &isis_ctp->task_truesender,
					FTYPE_ADDRESS, sizeof(address));
		BEGINFROMC if (fifo == 0)
		...
		ENDFROMC if ((options & ISISBC_MSEND) == 0)
			msg_delete(rmsg);
	} else
