...
static void
write_entry(rcfile, pgname, program, argv, envp)
	FILE *rcfile;
	char *pgname;
	char *program;
	char *argv[], *envp[];
	...
	char *argv[], *envp[];
{
	...
	/*** skip to end of line ***/
		while (buffer[strlen(buffer) - 1] != '\n') {
			if (fgets(buffer, BUFSIZE, rcfile) == NULL)
				break;
		}
	}
	if (entry_no < 0) {
	...
	message *mp;
{
	...
	msg_get(mp, "%d", &site);
	if (rmgr_optv)
		printf("rmgr_mh_rmup: news from site %d\n", site);
	if (slist[site]) {
		news_count++;
		t_sig(&rmup_cond, 0);
	}
...
#define  vv  vlist[i]
	...
	/*** build alist from slist ***/
		...
		for (i = 0; i < MAX_SITES; i++) {
			if (slist[i]) {
				alist[ns++] =
				    ADDRESS(i, (int) sv->sv_incarn[i], RMGR, MSG_LASTVIEW);
			}
		}
	...
	struct log *lt;
{
	...
	if (cc = sizeof(rmgr_viewlog)) {
		if (rlog.rl_view.gv_incarn == pg->gv_incarn
			...
			/* view has already been written by another member of the group at this
			   site */
			return;
		}
	} else if (cc != 0) {
	...
	char *key;
{
    ...
    /*** cancel pg monitor ***/
	if (pg_monitor_cancel(logtable[i].mid) != 0)
		return -1;
	...
	lognext--;
	if (lognext > i)
		logtable[i] = logtable[lognext];

	return 0;
}

int
rmgr_lasttofail(gname, key, old_incarn_p, noblock)
	char *gname, *key;
	int *old_incarn_p;
	int noblock;
{
	...
	rmgr_info *rmi;
{
	...
	sprintf(subj, "RM_NEWS:%s", rmi->rm_view.gv_name);
	if (news_subscribe(subj, GENERIC_RM_NEWS, 1) < 0)
		return &NULLADDRESS;
