...
#define xid_cmp(id1, id2) addr_cmp(id1, id2)
  /* Returns 0 if id1 and id2 are equal, > 0 if id1 is "greater than" id2, and < 0 if id1 is "less
     than" id2. x_id_cmp is NOT A SERIALIZATION ORDERING on transactions!!! It's intended for
     equality tests, and as a key in sorting and table lookup. */
...
#define X_COMPUTING -1		/* Transaction in progress (i.e. after x_begin but before x_commit
				   or x_abort). */
#define X_PREPARE   0		/* In phase 1 of a 2 phase commit. */
#define X_COMMIT    1		/* Committed, but not necessarily all participants know this yet. */
#define X_ABORT     2		/* Aborted, but not necessarily all participants know yet. */
#define X_FINISHED  3		/* All participants know the outcome, so we can delete information
...
extern "C" {
	...
	int x_term_msg(char *participant_name,
		       ...
		       bool (*on_commit) (x_id * id), bool (*on_abort) (x_id * id), message * data);
	...
	int x_term(char *participant_name,
		   ifunc * on_prepare, ifunc * on_commit, ifunc * on_abort, char *fmt ...);
#else
	int x_term();
#endif
