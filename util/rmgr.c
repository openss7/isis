...
char rmgr_rcsid[] =
    "$Revision: 2.6 $$Date: 90/06/20 14:36:28 $$Source: /usr/fsys/isisfsys/b/isis/isisv2.1/util/RCS/rmgr.c,v $";
...
#define reteof  if (feof(rcfile)) return EOF
{
	...
	while ((c = fgetc(rcfile)) != '"') {
		while (c != '\n') {
			if ((c = fgetc(rcfile)) == EOF)
				return EOF;
		}
	}
	s = argbuf;
	while ((*s++ = fgetc(rcfile)) != '"')
		reteof;
	*(s - 1) = '\0';
	key = argbuf;
 ...
 */
{
	...
	isis_start_done();
	if (optv)
		printf("rmgr %d: initial restart sequence started\n", my_site_no);
 ...
 */
{
	...
	if (strcmp(argv[0], "echo") == 0) {
		...
		sprintf(answer = buffer, "Commands: echo, status, dump, +v, -v, help.\n");
 ...
 */
{
	...
	if (lock < 0) {
		fprintf(stderr, "rmgr %d: rmgr.rc file seems to be deadlocked\n", my_site_no);
	}
