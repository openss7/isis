...
char news_rcsid[] =
    "$Revision: 2.6 $$Date: 90/06/20 14:36:41 $$Source: /usr/fsys/isisfsys/b/isis/isisv2.1/util/RCS/news.c,v $";
	...
	register char *subj;
{
	...
	for (stp = subj_table; stp < subj_table + MAXSUBJ; stp++) {
		if (stp->used && strcmp(stp->subj, subj) == 0)
			return stp;
	}
	return NULL;
}
 ...
 */
{
	...
	if (strcmp(argv[0], "echo") == 0) {
		...
		sprintf(answer = buffer, "Commands: echo, status, +v, -v, +vv, -vv, help.\n");
	...
	while (answer-- > 0) {
		if (optvv)
			printf("news %d: forwarding back issue \"%s\" to %d/%d:%d/%d\n",
			       my_site_no, subj,
			       alist->addr_site, alist->addr_incarn, alist->addr_process,
			       alist->addr_entry);
 ...
 */
{
	...
	/*** remove subscription from subscr list ***/
		...
		stp->nsubs--;
		if (stp->nsubs > subi)
			stp->subscr[subi] = stp->subscr[stp->nsubs];
