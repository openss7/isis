...
char rexec_rcsid[] =
    "$Revision: 2.16 $$Date: 90/08/07 16:05:42 $$Source: /usr/fsys/isisfsys/b/isis/isisv2.1/util/RCS/rexec.c,v $";
#include "isis.h"
#include "rexec.h"
#include "string.h"
#include <errno.h>
	...
	char **argv;
{
	...
	if (!getuid()) {
		fprintf(rfile,
			"rexec running as root and FORK_BROKEN, so can't setuid() ... aborting\n");
	...
	register message *mp;
{
...
#ifdef REXEC_REQUIRE_SETUID
	{
		if (getuid() != pwd->pw_uid) {
			fprintf(rfile,
				"... rexec failed: REXEC_REQUIRE_SETUID, but can't setuid() (FORK_BROKEN)\n");
			fprintf(stderr, "<isis-rexec> unable to execute %s\n", path);
			pname = NULLADDRESS;
		} else
			pname.addr_process = isis_fork_execve(path, args, env, fileno(rfile), -1);
	}
...
#else				/* not FORK_BROKEN */
	{
...
#ifdef REXEC_REQUIRE_SETUID
		{
			if (getuid() != pwd->pw_uid) {
				fprintf(rfile,
					"... rexec failed: REXEC_REQUIRE_SETUID, but can't setuid()\n");
				fprintf(stderr, "<isis-rexec> unable to execute %s\n", path);
				pname = NULLADDRESS;
			} else
				pname.addr_process =
...
struct passwd *
authenticate(user, hostname, passwd)
	...
	char *name, *program, **argp, **envp;
{
	...
	if ((gid != getgid()) || (gid != getegid())) {
		if (setgid(gid) == -1) {
			fprintf(stderr, "ISIS: unable to exec <%s>: setgid(%d)", program, gid);
			perror("");
			exit(0);
		}
		initgroups(name, gid);
	...
	if ((uid != getuid()) || (gid != getgid()))
		if (setuid(uid) == -1) {
			fprintf(stderr, "ISIS: unable to exec <%s>: setuid(%d)", program, uid);
			perror("");
			exit(0);
		}
	if (envp != (char **) 0)
