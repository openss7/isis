 ...
 */
char prstat_rcsid[] =
    "$Revision: 2.15 $$Date: 90/08/06 13:48:58 $$Source: /usr/fsys/isisfsys/b/isis/isisv2.1/util/RCS/prstat.c,v $";
#include "isis.h"
...
main_loop()
{
	...
	printf
	    ("REP = messages system->clients.  FA = cached view faults.  LK = namespace lookups\n");
	printf("VC = View changes.  IS = Intersite packets sent.  IG = Intersite received.\n");
	printf
	    ("IM = Valid non-dup non-ack messages. NL, ND = Local/total deletable astore entries\n");
...
pstats()
{
...
#else
	printf("%.2d:%.2d.%.2d%3d%5d%5d%4d%4d%3d%3d%3d%3d%3d%2d.%1d", 0, 0, 0,
#endif				/* (SUN || AUX) */
	       new_sbuf.is_ntasks,
	       dt.is_stats[S_NFORK],
	       ...
	       dt.is_stats[S_GBABORTS], fanouth, fanoutl);
	printf("%4d%4d%4d%4d%3d%4d%4d%3d%3d%3d%4d%4d%4d%3d%3d%3d  ",
	       ...
	       dt.is_stats[S_IGOT],
	       dt.is_stats[S_IMSGS],
	       new_sbuf.is_nlocdelete, new_sbuf.is_ndelete, dt.is_stats[S_CONGEST]);
	if (new_sbuf.is_congest & IS_CONGEST) {
		...
		printf("    ");
	printf("%4d%4d%4d\n",
	       new_sbuf.is_memuse / 1000, new_sbuf.is_msgmem / 1000, new_sbuf.is_inter / 1000);
	fflush(stdout);
}
