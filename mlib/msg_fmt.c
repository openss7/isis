...
#ifdef FUN_TYPES
int _msg_doputf(message * mp, char *format, int fname, va_list * ap, char **dp);
		...
		if (c == '%') {
			...
			default:
				if (vector) {
					...
					vfunc *dealloc = (by_ref ? VA_REF(*ap, vfunc *) : 0);

					len *= fi->fi_size;
					if (!by_ref || len < 64) {
						msg_insertfield(mp, fname, (char *) where,
								fi->fi_type, len);
						/* Make sure that data deallocator is called */
						if (dealloc)
							(*dealloc) ((char *) where);
					} else
						msg_insertref(mp, fname, (char *) where,
							      fi->fi_type, len, dealloc);
				} else
					switch (c) {
					case 'c':
					case 'd':
					...
					case 'h':{
						...
						if (c == 'h') {
							...
							msg_insertfield(mp, fname,
									(char *) &is_short,
									FTYPE_SHORT, sizeof(short));
						} else if (c == 'c') {
							char is_char = is_int;

							msg_insertfield(mp, fname, &is_char,
									FTYPE_CHAR, sizeof(char));
						} else
							msg_insertfield(mp, fname, (char *) &is_int,
									FTYPE_LONG, sizeof(long));
						break;
					}
					default:
						print
						    ("msg_put: %%%c x-by-value not supported (use %%%c[1] x-by-ref)\n",
						     c, toupper(c));
			...
			case FSTRING:
				if (vector) {
					...
					msg_insertfield(mp, fname, is_str, fi->fi_type,
							strlen(is_str) + 1);
				}
				break;
			}
		...
		while (c = *format++)
			if (c == '%') {
					...
					if (do_malloc) {
						...
						*(*(char ***) ptr)++ = dest;
						...
						if ((isis_state & ISIS_XBYREF)
							...
							|| c == 'e'))
							panic
							    ("msg_get: %%%c not implemented from fortran (use %%-%c)",
							     c, c);
						if (len)
				...
				case FMESSAGE:
					...
					*(*(message ***) ptr)++ =
					    msg_getmsg(mp, fname, (*msg_fpointer)++);
					break;
				}
				dp = ptr;
			} else if (c == '{') {
				...
				*fcp = 0;
				...
				while (count--) {
					...
					rval =
					    _msg_dogetf(&msg, 1, fname, fpt, (va_list *) 0, fcopy,
							&datap);
					if (rval == -1)
						return (-1);
					getf_count += rval;
