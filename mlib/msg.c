	...
	if (--hdr->hdr_nlinks == 0) {
		...
		if (hdr->hdr_flags & MSG_XFIELDS)
			for (fp = mp->msg_fields; fp; fp = fp->fld_next)
				...
				else if (fp->fld_type == FTYPE_FIELDREF
					 && is_field_ref(fp)->fr_routine)
					if (is_field_ref(fp)->fr_xref)
						ISISCALL1(is_field_ref(fp)->fr_routine,
							  &is_field_ref(fp)->fr_ptr);
					else
						ISISCALL1(*is_field_ref(fp)->fr_routine,
							  is_field_ref(fp)->fr_ptr);
	...
	vfunc *routine;
{
	...
	fp = &((msg_fld *)
	       msg_insertfield(mp, name, (char *) &fieldref, FTYPE_FIELDREF, sizeof(fieldref)))[-1];
	...
	register message *mp;
{
	...
	if (hdr->hdr_nlinks > 1)
		print("PSEUDO_");
	print("MSG %x: len %d refs %d flag 0x%.2x, nlinks %d", mp, mp->msg_len, mp->msg_refs,
	      hdr->hdr_flags, hdr->hdr_nlinks);
	if (mp->msg_base)
		print(", base %x", mp->msg_base);
	print("\nSender ");
	paddr(&hdr->hdr_sender);
	print(", msgid %d", hdr->hdr_msgid);
	if (hdr->hdr_msgid & 1)
		print(", replyto ");
	paddr(&hdr->hdr_replyto);
	...
	register msg_fld *fp;
{
	...
	default:
		print("  Field 0x%.2x: <%s> len %d at %x\n", fp->fld_name,
		      msg_typename(fp->fld_type), fp->fld_len, &fp[1]);
		break;
	case FTYPE_DELETED:
		break;
	case FTYPE_MESSAGE:
		print("  Field 0x%.2x: fld_len %d REF <message> %x\n", fp->fld_name, fp->fld_len,
		      is_msg_ref(fp)->mr_msg);
		break;
	case FTYPE_FIELDREF:
	begin {
		...
		print("  Field 0x%.2x: fld_len %d REF <%s> len %d routine %x data %x\n",
		      fp->fld_name, fp->fld_len, msg_typename(fr->fr_type), fr->fr_len,
		      fr->fr_routine, fr->fr_ptr);
	}
	}
}

static void
pblks(bp, len, mlen, base)
	register block_desc *bp;
	register mlen, len;
	register char *base;
{
	...
	if (fp)
		do
			...
			begin {
				...
				nfp = &((msg_fld *)
					msg_insertfield(mp, fp->fld_name, (char *) &fp[1],
							fp->fld_type, fp->fld_len))[-1];
				fref_register(fr);
				...
				continue;
			default:
				msg_insertfield(mp, fp->fld_name, (char *) &fp[1], fp->fld_type,
						fp->fld_len);
				continue;
			}
...
#define dfp      ((msg_fld*)data)
	...
	while (*(int *) hdr->hdr_version != isis_version.LONG) {
		...
		paddr(&my_address);
		print
		    (": msg_reconstruct -- invalid message or from incompatible version of ISIS (%s)\n",
		     hdr->hdr_version);
		msg_delete(mp);
		return ((message *) 0);
	}
	if (convert =
	    (hdr->hdr_shortcode != MSG_SHORTCODE || hdr->hdr_longcode != MSG_LONGCODE
	     || hdr->hdr_dblcode != MSG_DBLCODE)) {
	...
	while (len) {
		...
		if (fld_len < 0 || fld_len > len) {
			print
			    ("WARNING: msg_doreconstruct: message field has impossible length %d\n",
			     fld_len);
			msg_delete(mp);
			return ((message *) 0);
		}
