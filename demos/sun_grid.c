	...
	char **argv;
{
	...
	base_frame = (Frame *) window_create(NULL, FRAME,
					     ...
					     WIN_X, 2,
					     WIN_Y, 2, WIN_ERROR_MSG, "Cannot create frame", 0);
	...
	grid_panel = (Panel *) window_create(base_frame, PANEL,
					     ...
					     WIN_HEIGHT, height / 4,
					     WIN_X, 0,
					     WIN_Y, height + 4, PANEL_BLINK_CARET, FALSE, 0);
	...
	grid_canvas = (Canvas *) window_create(base_frame, CANVAS,
					       ...
					       WIN_WIDTH, width,
					       WIN_HEIGHT, height, WIN_X, 0, WIN_Y, 0, 0);
...
control_panel_layout()
{
	...
	while (bi->bu_text) {
		(void) panel_create_item(grid_panel, PANEL_BUTTON,
					 PANEL_LABEL_IMAGE, panel_button_image(grid_panel,
									       bi->bu_text, 8,
									       screen_r_11),
					 PANEL_NOTIFY_PROC, bi->bu_proc, PANEL_LABEL_X, bi->bu_x,
	...
	Bcast = panel_create_item(grid_panel, PANEL_CHOICE,
				  ...
				  PANEL_ITEM_X, PANEL_CU(0), PANEL_ITEM_Y, PANEL_CU(1), 0);
	StopGo = panel_create_item(grid_panel, PANEL_CHOICE,
				   ...
				   PANEL_ITEM_X, PANEL_CU(10), PANEL_ITEM_Y, PANEL_CU(0), 0);
	(void) panel_create_item(grid_panel, PANEL_CHOICE,
				 ...
				 PANEL_ITEM_X, PANEL_CU(20), PANEL_ITEM_Y, PANEL_CU(0), 0);
	Members = panel_create_item(grid_panel, PANEL_MESSAGE,
				    PANEL_LABEL_STRING, "Ready to join...",
				    PANEL_ITEM_X, PANEL_CU(0), PANEL_ITEM_Y, PANEL_CU(2) + 2, 0);
	Speed = panel_create_item(grid_panel, PANEL_SLIDER,
				  ...
				  PANEL_MIN_VALUE, 0,
				  PANEL_MAX_VALUE, 250,
				  PANEL_ITEM_X, PANEL_CU(0), PANEL_ITEM_Y, PANEL_CU(3), 0);
}
...
print_value()
{
	...
	for (i = 0; i < 8; i++) {
		...
		sgprint("%.2d.%d %.2d.%d %.2d.%d %.2d.%d ", v[0], c[0], v[1], c[1], v[2], c[2],
			v[3], c[3]);
		sgprint("%.2d.%d %.2d.%d %.2d.%d %.2d.%d\n", v[4], c[4], v[5], c[5], v[6], c[6],
			v[7], c[7]);
	}
}
	...
	int i, j;
{
	...
	if (oldcolor[i][j])
		pw_writebackground(gridpw, 2 + width / 8 * i, 2 + height / 8 * j, width / 8 - 3,
				   height / 8 - 3, PIX_WHITE);
	pw_text(gridpw, width / 8 * i + width / 16, height / 8 * j + height / 16, PIX_BLACK,
		screen_r_11, string);
	if (color[i][j])
		pw_writebackground(gridpw, 2 + width / 8 * i, 2 + height / 8 * j, width / 8 - 3,
				   height / 8 - 3, PIX_REVERSE);
	...
	int loc;
{
	...
	print_value();
	if (loc == -1)
		xfer_out(loc, "%L%C%d%d", value[0], sizeof(value) / sizeof(int), color[0],
			 sizeof(color), mode, go);
