	...
	int x, y;
{
	...
	win = XCreateSimpleWindow(dpy, RootWindow(dpy, screen),
				  x, y, windwidth, windheight, bdrwidth, black, white);
	/* The x and y positions here don't seem to do anything: the window manager grabs control
	...
	int x, y;
{
	...
	XDrawImageString(dpy, win, gc,
			 ...
			 by + charheight + font->ascent, data, dlen);
...
display_title()
{
	...
	str = (mode == FB_MODE ? "fbcast" :
	       (mode == AB_MODE ? "abcast" :
		(mode == CB_MODE ? "cbcast" :
		 (mode == MB_MODE ? "mbcast" : (mode == GB_MODE ? "gbcast" : "random")))));
	XDrawImageString(dpy, win, gc, 0 * charwidth + 5, charheight + font->ascent, str, 6);
	...
	if (joined && n_memb > 0) {
		...
		XDrawImageString(dpy, win, gc, 7 * charwidth + 3,
				 charheight + font->ascent, string, 12);
	}
	...
	XDrawImageString(dpy, win, gc, 37 * charwidth + 7, charheight + font->ascent, str, 4);

	str = (joined ? "exit" : "join");
	XDrawImageString(dpy, win, gc, 43 * charwidth + 6, charheight + font->ascent, str, 4);
		...
		if (((XButtonEvent *) & event)->window == win)
			if (event.type == Expose)
				...
				if (((XButtonEvent *) & event)->y < titleheight) {
					...
					x = (x - 5) / charwidth;
					if (x < 48) {
						/* [stream: 25-30 use_rpc 31-35] stop: 37-42 exit:
						   43-48 */
						if (x < 10)
