	...
	int *countptr, nb;
{
	...
	while (nb) {
		val = ++*countptr;
		if (net != (char *) 0)
			spool(groupName, RCV_MESS_WITH_SEQN, "%d%s%s%d%s", 3, sender, groupName,
			      val, key, SP_NETWORK, net, SP_ID, val, SP_KEYWORDS, key, 0, 0);
		else
			spool(groupName, RCV_MESS_WITH_SEQN, "%d%s%s%d%s", 3, sender, groupName,
			      val, key, SP_ID, val, SP_KEYWORDS, key, 0, 0);
		--nb;
	}
}
...
{
	spool_set_replay_pointer(groupList[gid].name, groupList[gid].spseqn);
	spool_and_discard(groupList[gid].name, CHECKPOINT, "%s%d%d%s",
			  "keyed and selected checpoint: ", groupList[gid].spseqn, messID, key,
			  SP_KEYWORDS, "CHECKPOINT", key, 0, 0, SP_KEYWORDS, key, 0, SP_ID,
			  SP_INFINITY, messID, 0);
}
	...
	if (isalpha(car))
			...
			for (i = 0; i < MAXTESTGROUPS; i++)
				if (groupList[i].spseqn != 0) {
					printf("Setting replay pointer of spool: ** %s ** \n",
					       groupList[i].name);
					spool_set_replay_pointer(groupList[i].name,
								 groupList[i].spseqn);
		...
		case 0:
			...
			if (!groupList[gid].spseqn) {
				printf
				    ("Current spool sequence numeber is zero. Give the initial value ? ");
				scanf("%d", &ptrVal);
			} else
