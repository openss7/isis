...
int graph_err;				/* 0 if graph built since init. is consistent: 1 - data
					   item output by more than 1 step 2 - cycle in graph 3 -
					   graph has too many items for alloc. index */
#define GRAPH_OK 0
...
*/
{
	...
	env_ptr cur_env;		/* pointer to our current position in the environment array 
					 */

	/* count the strings */
	for (cur_node = p_env->head; cur_node; cur_node = cur_node->next)
		num_strings++;
...
bool
envcmp(p_env1, p_env2)
	env_ptr p_env1, p_env2;

     /* compare the 2 environments and return 0 if equal, or result of first non-0 strcmp */
{
	int result;
	...
	if (p_env1 && p_env2) {
		for (; *p_env1 && *p_env2;)
			if (result = strcmp(*(p_env1++), *(p_env2++)))
				return result;

		if (*p_env1 || *p_env2)
			return 1;	/* one ended before other */
		else
			return 0;
	} else
		return 1;	/* one was totally nil. */
}
...
*/
{
	if (step_addr) {	/* index allocated */
		if (p_step->step_num < max_steps)
			(*step_addr)[p_step->step_num] = p_step;
		else
			graph_err = INDEX_GRAPH_ERR;
	}
}

void
set_item_addr(p_item)
	data_item *p_item;
...
*/
{
	if (server_addr) {	/* index allocated */
...
void
alloc_index(p_items, p_steps, p_servers)
...
data_item *
new_data(p_name, p_type, p_size, p_changed)
	char *p_name;
	dtype p_type;
	int p_size;
	bool p_changed;
...

{
	...
	if (!new_node)
		printf("no memory for data node");
	else {
...
*/
{
	...
	if (!new_node)
		printf("no memory for graph node");
	else {
		/* set name and number */
		...
		new_node->calc.duration = new_node->calc.cpath_time = new_node->calc.dep_time = 0;
   ...
   */
{
	...
	for (cur_node = data_list.head; cur_node; cur_node = data_list.head) {
		for (ref_node = ((data_item *) cur_node->content)->dependers.head;
		     ref_node; ref_node = ((data_item *) cur_node->content)->dependers.head)
			delete(ref_node, &(((data_item *) cur_node->content)->dependers));

		for (ref_node = ((data_item *) cur_node->content)->gen_deps.head;
		     ref_node; ref_node = ((data_item *) cur_node->content)->gen_deps.head)
	...
	/* imports */
	for (dequeue(&imports); imports.head; dequeue(&imports))
		continue;
	...
	for (cur_node = step_list.head; cur_node; cur_node = step_list.head) {
		for (ref_node = ((graph_node *) cur_node->content)->in_list.head;
		     ref_node; ref_node = ((graph_node *) cur_node->content)->in_list.head)
			delete(ref_node, &(((graph_node *) cur_node->content)->in_list));

		for (ref_node = ((graph_node *) cur_node->content)->output.head;
		     ref_node; ref_node = ((graph_node *) cur_node->content)->output.head)
	...
	for (cur_node = servers.head; cur_node; cur_node = servers.head) {
		for (ref_node = ((server_type *) cur_node->content)->queue.head;
		     ref_node; ref_node = ((server_type *) cur_node->content)->queue.head)
...
{
#ifdef dbggph
	printf("in dep %d at %x on %d at %x\n", p_step->step_num, p_step, p_data->data_num, p_data);
#endif
...
*/
{
	...
	if (!p_data->source) {	/* only add dep. if no other node outputs p_data */
		...
		graph_err = MULTI_OUT_GRAPH_ERR;

}

void
add_gen_dep(p_dep, p_data)
...
{
#ifdef dbggph
	printf("gen dep %d at %x on %d at %x\n", p_data->data_num, p_data, p_dep->data_num, p_dep);
#endif
...

{
	...
	val1 = val2 = 0;
	if (p_node1)
		val1 = ((graph_node *) p_node1)->step_num;
	if (p_node2)
		val2 = ((graph_node *) p_node2)->step_num;
 ...
 /* returns TRUE if graph is acyclic, else FALSE */
{
	...
	/* initialize cur_dep to num_deps for each step.  Also, enqueue any nodes with no input
	...
	/* decrement cur_dep for any step that depends on an imported data item. enqueue and step
	   whose cur_dep goes to 0. */
...
#endif
			if (((graph_node *) cur_step->content)->cur_dep) {	/* not all deps
				   found *//* if dependency changed, recompute step */
				...
				if (!(--((graph_node *) cur_step->content)->cur_dep)) {	/* number
											   dependers=0. 
											   assign
											   number
											   and
	...
	for (cur_gnode = ((graph_node *) dequeue(&proc_Q));
		...
		if (cur_gnode->output.head) {
			for (cur_data = cur_gnode->output.head; cur_data; cur_data = cur_data->next) {
#ifdef dbggph
				...
				for (cur_step = ((data_item *) cur_data->content)->dependers.head;
					...
					if (((graph_node *) cur_step->content)->cur_dep) {	/* not 
												   all 
												   deps 
												   found 
...
#endif
							((graph_node *) cur_step->
							 content)->step_num = next_number++;
							set_step_addr((graph_node *)
								      cur_step->content);
							enqueue(cur_step->content, &proc_Q);
	...
	/* having processed all nodes whose indegree went to 0 we will have hit all steps unless
	   there was a cycle */
	...
	/* time to start sorting lists.  The sort procedure in lst accepts a list and a pointer to
	   a 2 parameter function used to compare 2 list elements. */
	/* first the step list */
...
*/
{
	...
	list_node *step_node, *dep_node;	/* list nodes pointing to step being evaluated and
						   depender. */
	...
	for (step_node = step_list.tail; step_node; step_node = step_node->last) {
		...
		if (cur_step->calc.recompute == TRUE) {
			...
			my_cpath = 0;
			my_dep = 0;
			for (data_node = cur_step->output.head; data_node;
				...
				for (dep_node = ((data_item *) data_node->content)->dependers.head;
					...
					my_dep +=
					    visit_step->calc.dep_time + visit_step->calc.duration;
