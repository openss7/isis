...
typedef struct {
	int entry_state;		/* current state represented */
...
automatype dep_grammar[LAST_STATE + 1] = {
	...
	LAST_STATE, 'A', RTN_DONE, LAST_STATE
};				/* last state marker - action meaningless */

int
get_dep_word(p_word, p_init_string)
	char **p_word;			/* return pointer to next word */
	char *p_init_string;		/* initial dependency description string - used as
					   workspace */
...
*/
{
	static char saved_char[2];	/* delimiter of last p_word returned, temporarily replaced
					   by \0 in input string. */
	...
	if (saved_char[0] != '\0') {	/* more string to scan.  restore saved character. */
		cur_string[cur_pos] = saved_char[0];

		do {		/* loop for running automaton.  Find current input in current state 
				 */
			/* increment search state until the test character equals the next
			   character in the string, or the next dep_grammar entry to be scanned is
			   associated with a different current state. */
			...
			case RTN_IN:
			{
				/* save current character and replace with \0, then return
				   IN_DEP_FILE */
			...
			{	/* just increment current position */
				/* the only diff. between ACCUM and SKIP is whether START was
				   called previously, posting the start pointer. */
				cur_pos++;
				break;
			}
	...
	} else {		/* saved character was \0.  no input string */
		return (DEP_DONE);
	}
}
...
real_name(p_file)
	char *p_file;
     ...
     /* return the real name of the indicated file given the current directory in my_dir
        preconditions : my_dir set.  NO ESCAPED /'s IN P_FILE !!!! postconditions : pointer to
        newly allocated string returned unless name has inconsistent /../'s */
{
	...
	*prev_slash;			/* pointer to / precedeing a /../ */
	...
	if (!strncmp(p_file, "/", 1))
		return (strcpy(new_name, p_file));

	/* if name begins with ./, remove it. either way, prepend current directory */
	strcpy(new_name, my_dir);
	strcat(new_name, "/");
	if (!strncmp(p_file, "./", 2))
		strcat(new_name, p_file + 2);
	else
		strcat(new_name, p_file);

	/* for every /../ imbedded in the file name, remove the directory name immediately
	   precedeing it.  */
   ...
   p_stream.  Preceed the command with the keyword STEP.  */
{
	...
	for (cur_arg = 4; cur_arg < p_argc; ++cur_arg) {	/* add argv[cur_arg] to cmd_string
...

{
	...
	if (!stat(p_fname, &stat_buf)) {	/* got it.  just return it. */
...
void
eval_unk(p_graph_file, p_argc, p_argv, p_cmd_arg)
	int p_argc, p_cmd_arg;		/* number of pmksh arguements and the index of the one to
					   be evaluated */
...
*/
{
	...
	/* initialize: nullify output name and make a working copy of the dependency string */

	strcpy(out_file_name, "");
	strcpy(scratch, p_argv[3]);
	input_word_type = get_dep_word(&input_word, scratch);

	do {			/* as long as input lasts *//* respond to the input type received */
		...
		case OUT_DEP_FILE:
		{		/* the char string for unknown commands is the output file size and 
			...
			if ((input_word_type = get_dep_word(&input_word, NULL)) == CHAR_STRING) {
				/* expect 2 fields: outfile size and exec. time. if there is only 1 
				   field, use UNIT_TIME for exec. time */
				...
				if (input_size <= 0) {
					...
					append_time(p_graph_file, UNIT_TIME);
				} else {	/* make a wild guess at output size and execution
						   time */
...
typedef struct {
	...
	float out_coeff,		/* ratio of input sz to output contribution for this suffix 
					 */
	 time_coeff;			/* ratio of input size to ms of contribution to exec. time */
} lin_suff_type;
...
bool
is_suff(p_word, p_suffix)
	char *p_word, *p_suffix;

/* return TRUE if p_suffix is a suffix of p_word,
   FALSE otherwise.  */
{
	int word_pos, suffix_pos;	/* current position in word and suffix respectively. */

	/* start at the end */
	word_pos = strlen(p_word) - 1;

	/* fail if any characters mismatch or word ends before suffix. */
	for (suffix_pos = strlen(p_suffix) - 1; suffix_pos >= 0; suffix_pos--) {
		if ((word_pos < 0) || (p_word[word_pos] != p_suffix[suffix_pos]))
			return (FALSE);
		word_pos--;
	}
	return (TRUE);
		...
		{		/* look for the file */
			...
			for (cur_suffix = 0; (cur_suffix < p_num_suffs) &&
				...
				if (is_suff(input_word, p_suff_map[cur_suffix].suffix)) {
					...
					else {	/* tack the directory onto the input word */
			...
			/* now infile_sz is either the size of the file if found, or -1 if not.  If 
			   file found, fname contains the full name. */
			if (infile_sz == -1)
				append_in(p_graph_file, real_name(input_word), -1);
			else {	/* post the actual input file name found and accumulate output size 
				   and time figures */
...
void
eval_cc(p_graph_file, p_argc, p_argv, p_cmd_arg)
...
typedef struct {
	char cmd_name[fname_sz];	/* a command recognized */
	void (*eval_cmd) ();		/* pointer to routine used to evaluate the command.  */
} cmd_lib_node;
...
*/
{
	...
	if (p_cmd_arg < p_argc)
		...
		strcpy(cmd_to_find, "");	/* if command after last arg. use "" to indicate no 
						   command at all. */

	for (cur_node = cmd_lib.head; cur_node; cur_node = (list_node *) (cur_node->next)) {
		if (!strcmp(cmd_to_find, ((cmd_lib_node *) cur_node->content)->cmd_name)) {	/* command 
												   found 
												   execute 
												   associated 
												   evaluation 
