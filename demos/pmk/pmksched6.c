...
struct ctime_node_type {		/* node describing all ready steps with a given
					   ctime-duration */
	int tag_time,			/* ctime-dur value for this node */
	 steps,				/* number of steps */
	 total_comp;			/* total compute time for all steps in this node */
...
bool rebuild_rt;			/* indicates that real time structures must be rebuilt */
        ...
        */
{
	...
	if (my_next_node && (((graph_node *) my_next_node->content)->calc.status == SCHEDULED)) {
		*p_node = my_next_node;
		*p_step = (graph_node *) my_next_node->content;
		return START_EXEC;
	}

	/* Simple minded part: look for work immediately based on the following: 1- MOVE_AND_EXEC
	   the first SCHEDULED step in my queue. 2- TAKE the first SCHEDULED step located by
	   scanning through server's queues beginning from the last server in the server list. 3-
	   Steps whose transfers have been denied less than RE_REQ_DELAY ticks ago are not eligible 
	   for transfer.
	...
	/* dull, dull, dull.  nothing to steal.  If I have more work in my queue, just wait for it. 
	 */
	if (my_next_node)
		return WAIT_PREREQ;
...
ctime_add(p_step)
	graph_node *p_step;		/* step to be added to ctime_list */
     ...
     /* add the indicated step to ctime_list.  ctime_list must have been initialized sometime in
        the past. p_step->calc must be set up. */
{
	...
	for (cur_node = ctime_list.head; cur_node &&
	     (((ctime_node *) cur_node->content)->tag_time != targ_time); cur_node = cur_node->next)
		continue;
	...
	dbgp "add step %d ctime-dur %d: steps=%d total_comp %d\n",
	    p_step->step_num, cur_ctime->tag_time, cur_ctime->steps, cur_ctime->total_comp dbge;
...
ctime_delete(p_step)
	graph_node *p_step;		/* step to be removed from ctime_list */
     ...
     /* remove p_step from the ctime_list structure.  If p_step was the last step in a node, delete 
        the node. precondition : ctime_list initialized and p_step->calc set */

{
	...
	if (!cur_node)
		pmkerr("cant find step in ctime\n");

	cur_ctime = (ctime_node *) cur_node->content;
	step_node = (list_node *) find_node_in_list((gen_ptr) p_step, cur_ctime->step_list);
	if (!step_node)
		pmkerr("cant find step in ctime\n");
...
int
comp_sched_time()
     /* compute the current schedule_time as defined in algorithm description ctime_list must be
        defined */
{
	...
	for (cur_node = ctime_list.head; cur_node; cur_node = cur_node->next) {
		...
		if (cur_ctime->steps > cur_view.gv_nmemb) {
		...
		} else {	/* pick largest duration step and multiply by smallest sf. */
			...
			for (eval_node = cur_ctime->step_list.head; eval_node;
				...
				if (((graph_node *) eval_node->content)->calc.duration
				    > largest_dur)
					largest_dur =
					    ((graph_node *) eval_node->content)->calc.duration;

			if ((cur_ctime->tag_time + ((largest_dur + (min_sf < 1 ? 1 : 0))
						    * min_sf)) > sched_time)
				sched_time =
				    cur_ctime->tag_time + ((largest_dur + (min_sf < 1 ? 1 : 0))
							   * min_sf);
...
init_work_list()
    /* build a work list consisting of all SCHEDULED steps in the AVAIL_WORK server */
{
	...
	for (srvr_node = servers.head; srvr_node; srvr_node = srvr_node->next) {
		if (((server_type *) srvr_node->content)->rank == AVAIL_WORK) {
			for (step_node = ((server_type *) srvr_node->content)->queue.head;
				...
				if (((graph_node *) step_node->content)->calc.status == SCHEDULED) {
...
int
compute_skip_time(p_srvr)
	server_type *p_srvr;

     /* compute the amount of compute time to SKIP ( in cumulative step duration ) in order to
        reach the work assigned to p_srvr. preconditions : avail_work_time initialized, server_list 
        initialized postconditions : returns time to skip */
{
	...
	/* skip the amount of work executable by each server which is faster than p_srvr by more
	   than DIV_CONQ_THRESHOLD

	   Assuming that the average speed factor is always 1 ( not entirely accurate, but
	   expedient ) the work allocated to a given processor is avail_compute_time/(number of
	   servers * speed factor) */
...
give_work(p_srvr)
	int *p_srvr;			/* server to receive work */
     ...
     /* find the first server that doesn't have work to do, and give it the step with the latgest
        ctime that will fit between the calculated schedule time and the ctime-dur of the step.
        precondition: preschedule called and current scheduler is manager of work_list ( rank 0 )
        init_work_list ( or equivilent code in preschedule) called */
{
	...
	if (cur_srvr) {
		...
		for (give_node = work_list.head; give_node; give_node = give_node->next) {
			...
			/* we want to schedule this step if : schedule time - sf * duration >=
			   ctime - duration.  ( remember, time runs backwards ) the following
			   conditional tests that. */

			if ((computed_time = (sched_time - ((cur_srvr->speed_factor) *
							    (float) give_step->calc.duration) +
					      1)) >= (cmpl_goal =
						      give_step->calc.cpath_time -
						      give_step->calc.duration + COMPLETION_MARGIN))
				break;	/* schedule step */
			else {
...
/* read the raw graph and devise an initial schedule for it */
{
	...
	alloc_index(num_items, num_steps, (num_servers > MIN_SRVRS ? num_servers : MIN_SRVRS));
	...
	/* initialize my server my_server=(server_type *)new_proc("origin",&servers); */
	...
	for (cur_node = step_list.head; cur_node; cur_node = cur_node->next) {
		...
		if (cur_step->num_deps)
			cur_step->calc.status = ALLOCATED;
		else {
	...
	for (data_node = imports.head; data_node; data_node = data_node->next)
		for (cur_node = ((data_item *) data_node->content)->dependers.head;
			...
			if (!--((graph_node *) cur_node->content)->cur_dep) {
				...
				avail_work_time +=
				    ((graph_node *) cur_node->content)->calc.duration;
			}
