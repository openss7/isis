...
bool execute_psteps = TRUE;		/* execute steps or just accumulate in graph file */
...
{
	fprintf(stderr,
		"Usage: %s [-f makefile] [-G graphfile] [-P isis_port] [-S num_servers]\n                 [-inpqrstAK] [macro=val ...] [target(s) ...]\n",
		myname);
	exit(1);
}
