/*   $RCSfile: make.c,v $ $Revision: 2.1 $ $Date: 90/07/31 16:17:15 $ */
/*
...
extern FILE *pmake_stream;		/* stream for outputting parallel steps */
extern char **environ;			/* environment for parallel steps */
...
#endif
#ifdef eon
	return ((number = execl(shell, shell, "-c", string, 0)) == -1) ? -1 :	/* couldn't start
										   the shell */
...
void
pmksh_sub(graph_stream, p_env, p_deps, p_cmd)
	FILE *graph_stream;		/* the file opened in append mode to accept the graph */
	char **p_env;			/* environment to be appended if non-null */
	char *p_deps,			/* string describing dependencies */
	*p_cmd;				/* string containing command */

/*
  append the indicated command to the graph.
  Generate an agruement array which looks like the parameters to pmksh
  except that the graph file need not be named, and only the first word 
  of the command need be broken out.
  preconditions : graph file is opened for append on stream
*/
{
	int my_argc = 6;		/* size of arguement array */
	char **my_argv = (char **) malloc((my_argc + 1) * sizeof(char *));

	/* argument array with space for null termination */
	char null_string[2];

	null_string[0] = 0;

	my_argv[0] = null_string;	/* null strings for unused parms */
	my_argv[1] = null_string;
	my_argv[2] = null_string;
	my_argv[6] = NULL;	/* null pointer for termination */

	my_argv[3] = p_deps;
	my_argv[5] = p_cmd;
	my_argv[4] = gettok(&(my_argv[5]));	/* split out first command word */

	/* log the command in the graph description */
	append_cmd(graph_stream, my_argc, my_argv);

	/* evaluate the command in argv[4] */
	eval_lib_cmd(graph_stream, my_argc, my_argv, 4);

	if (p_env)
		append_env(graph_stream, p_env);

	append_end(graph_stream);
}
	...
	struct line *lp;
{
	...
	char cmd_str[line_sz];		/* new command line */
	char dep_str[line_sz];		/* dependency description */

	parallel = FALSE;
	cmd_str[0] = 0;		/* init to empty command and dependencys */
	dep_str[0] = 0;
	...
	for (cp = lp->l_cmd; cp; cp = cp->c_next) {
		...
		if (domake) {
			if ((q[0] != '|') && (parallel != TRUE)) {
				/* Serial command - Get the shell to execute it */
			...
			} else {	/* parallel command.  get rid of | */
				if (parallel == FALSE)
					strcat(cmd_str, &(q[1]));
				else
					strcat(cmd_str, q);
	...
	if (parallel == TRUE) {
		/* construct dependency description */
		struct depend *cur_dep;
		struct name *cur_targ;

		for (cur_dep = lp->l_dep; cur_dep; cur_dep = cur_dep->d_next) {
			strcat(dep_str, cur_dep->d_name->n_name);
			strcat(dep_str, " ");
		}

		for (cur_targ = np; cur_targ; cur_targ = cur_targ->same_rule) {
			strcat(dep_str, ">");
			strcat(dep_str, cur_targ->n_name);
		}
/*	    printf("deps %s for cmd %s\n",dep_str,cmd_str); */
		pmksh_sub(pmake_stream, environ, dep_str, cmd_str);
	}
}
