/*   $RCSfile: input.c,v $ $Revision: 2.1 $ $Date: 90/07/31 16:17:07 $ */
/*
 *	Parse a makefile
 */
	...
	char *name;
{
	...
	for (rp = namehead.n_next, rrp = &namehead; rp; rp = rp->n_next, rrp = rrp->n_next)
	...
	FILE *fd;
{
	...
	for (;;) {
		...
		while (((q = index(p, '=')) != (char *) 0) && (p != q) && (q[-1] == '\\')) {	/* Find 
												   value 
												 */
			register char *a;
		...
		*((q = str1) + strlen(str1) + 1) = '\0';
		...
		nnp = (struct name *) 0;	/* no previous name with same rule */
