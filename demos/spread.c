...
char grid_rcsid[] =
    "$Source: /usr/fsys/isisfsys/b/isis/isisv2.1/demos/RCS/spread.c,v $$Revision: 2.7 $$Date: 90/09/13 14:30:30 $";
	...
	char **argv;
{
		...
		if (**++argv >= '0' && **argv <= '9')
			...
			case 'n':	/* user wants a specific spreadsheet name */
				...
				if (strlen(spdshtname) > 10) {
					printf
					    ("Spreadsheets names are limited to ten characters, yours was shortened\n");
					spdshtname[11] = '\0';
				}
				strcpy(dspdshtname, "spreadsheet name : ");
				strcat(dspdshtname, spdshtname);
	...
	/* isis_input is called later in join_spd, this is to insure the user does not try to get a 
	   mouse click in before we have had a chance to read in all the spreadsheets. */
...
join()
{
	globalgaddr = pg_join("spreadsheet",
			      PG_INIT, create_global, PG_XFER, 0, send_global, receive_global, 0);
	get_join_group();
...
join_spd()
{
	...
	gaddr = pg_join(spreadgroup,
			...
			PG_MONITOR, change, 0, PG_XFER, 0, send_data, receive_data, 0);
	isis_start_done();
	...
	/* wait to here to call isis_input so as to prevent user from getting a mouse click in to
	   early see main() */
	isis_input(fdes, display_input, NULLARG);
	...
	register message *mp;
{
	...
	XFlush(dpy);
	nullreply(mp);
	/* should include reply to message only if a consistent external picture is desired, abcast 
	   guarantees a consistent internal picture */
	/* reply(mp,"%s","message received"); */
	...
	message *mp;
{
	...
	if (loc < MAX_SPDS_GRP) {
		...
		msg_get(mp, "%s%s%d%d",
			entry[adjustloc / 200][(adjustloc % 200) / 20][adjustloc % 20],
			display[adjustloc / 200][(adjustloc % 200) / 20][adjustloc % 20],
			&(entryval[adjustloc / 200][(adjustloc % 200) / 20][adjustloc % 20]),
			&(validentry[adjustloc / 200][(adjustloc % 200) / 20][adjustloc % 20]),
			(int *) 0);
	...
	address *gaddr;
{
	...
	if (merging == 1) {
		...
		if (spdshtname[0] == '\0') {
			error("spreadsheet name with 0 characters");
			/* strcpy(dspdshtname," "); strcpy(spdsht_list[0],"default");
			   strcpy(dspdsht_list[0],spdsht_list[0]); numb_spdshts = 1; */
		}
		if (spdshtname[0] != '\0') {
			strcpy(dspdshtname, "spreadsheet name : ");
			strcat(dspdshtname, spdshtname);
		...
		for (j = 0; j < 10; j++) {
			for (k = 0; k < 20; k++) {
				...
				if (entry[i][j][k][0] == '=') {
					...
					val = yyparse();
					if (del_dep == NULL)
						/* there is no entry in tht cell anymore make entry 
						   val = NULL */
	...
	int x, y;
{
	...
	if (dpy == NULL) {
		printf
		    ("Unable to establish xwindow connection, set display variable appropriately\n");
		exit(0);
	}
...
display_all()
{
	...
	/* XDrawRectangle(dpy,win,gc, 675,25,125,40); XDrawRectangle(dpy,win,gc, 676,26,123,38); */

	ChangeFont(SMALLFONT);
	XSetFont(dpy, gc, font->fid);
	for (i = 0; i < numb_spdshts; i++) {
		XDrawLine(dpy, win, gc, leftx + botwidth * i, topy + numbrows * rowwidth + botwidth * i, leftx + botwidth + botwidth * i, topy + numbrows * rowwidth + botwidth + botwidth * i);	/* bot 
																									   left 
																									   diag 
																									 */
		XDrawLine(dpy, win, gc, leftx + botwidth + botwidth * i, topy + numbrows * rowwidth + botwidth + botwidth * i, leftx + numbcols * colwidth + rtwidth + rtwidth * i, topy + numbrows * rowwidth + botwidth + botwidth * i);	/* bot 
																														   line 
																														 */
		XDrawLine(dpy, win, gc, leftx + numbcols * colwidth + rtwidth * i, topy + rtwidth * i, leftx + numbcols * colwidth + rtwidth + rtwidth * i, topy + rtwidth + rtwidth * i);	/* top 
																								   right 
																								   diag 
																								 */
		XDrawLine(dpy, win, gc, leftx + numbcols * colwidth + rtwidth + rtwidth * i, topy + rtwidth + rtwidth * i, leftx + numbcols * colwidth + rtwidth + rtwidth * i, topy + numbrows * rowwidth + botwidth + botwidth * i);	/* right 
																													   line 
																													 */
		XDrawLine(dpy, win, gc, leftx + numbcols * colwidth + rtwidth * i, topy + numbrows * rowwidth + botwidth * i, leftx + numbcols * colwidth + rtwidth + rtwidth * i, topy + numbrows * rowwidth + botwidth + botwidth * i);	/* bot 
																														   right 
																														   diag 
																														 */
		size = strlen(dspdsht_list[i]);
		XDrawImageString(dpy, win, gc, leftx + botwidth + 2 + botwidth * i,
				 topy + numbrows * rowwidth + 10 + botwidth * i, dspdsht_list[i],
				 size);
	}
	ChangeFont(NORMALFONT);
	if (demo_mode == 1)
...
display_title()
{
	...
	if (run_demo == 1) {
		if (demo_mode == 1) {
			XDrawImageString(dpy, win, gc, 500, 20,
					 "demo mode: Simulating and high-lighting", 39);
		} else {
	...
	XDrawLine(dpy, win, gc, menu[0].left - menu_margin, 0, menu[0].left - menu_margin,
		  menu_bot);
	for (i = 0; i < numb_menus; i++) {
		XDrawImageString(dpy, win, gc, menu[i].left, 10, menu[i].label,
				 strlen(menu[i].label));
		...
		if (((XButtonEvent *) & event)->window == win)
			if (event.type == Expose)
				...
				if ((((XButtonEvent *) & event)->y < (menu_bot)) &&
					...
					(menu[numb_menus].left - menu_margin))) {
		...
		if (((XButtonEvent *) & event)->window == win)
			if (event.type == Expose) {
				...
				/* check if mouse was click in menu bar */
					...
					BoxHilight = 0;
					if (demo_mode == 0) {
						XClearArea(dpy, win, 101 + oldc * 75,
							   101 + oldr * 25, 74, 24, False);
						size = strlen(display[dispdepth][column][row]);
						XDrawImageString(dpy, win, gc, 110 + 75 * oldc,
								 117 + 25 * oldr,
								 display[dispdepth][oldc][oldr],
								 min(size, 7));
					}
				}
			}
	...
	int ycoord;
{
	...
	XClearArea(dpy, win, leftx + 1, topy + numbrows * rowwidth + 1, 82, 10, False);
...
disp_inverse_text(c, r)
	int c, r;			/* the column, row we wish to display the inverse text
					   entry in */
{
	int size;
	char dispentry[32];
	...
	XDrawImageString(dpy, win, gc, 110 + 75 * column, 117 + 25 * row, dispentry, min(size, 7));
	XSetForeground(dpy, gc, black);
	XSetBackground(dpy, gc, white);
	...
	KeySym keysym;
{
	...
	if (keysym == XK_Delete) {
		if (BoxHilight == 1) {	/* if we are actually making an entry */
			...
			XClearArea(dpy, win, 200, 30, 800, 40, False);
			size = strlen(entry[dispdepth][column][row]);
			XDrawImageString(dpy, win, gc, 200, 50, entry[dispdepth][column][row],
					 min(size, 32));
			XFillRectangle(dpy, win, gc, 101 + column * 75, 101 + row * 25, 74, 24);
			disp_inverse_text(column, row);
		}
					 ...
					 */
{
	...
	while (search != NULL) {
		...
		if (demo_mode == 1 && dispdepth == cdep) {	/* display those values changed as
								   gray */
			temp = ((struct demo_element *) malloc(sizeof(struct demo_element)));
			temp->next = demo_list;
			temp->depth = cdep;
...
display_demo_list()
{
	...
	if (temp != NULL) {
		...
		while (temp != NULL) {
			XClearArea(dpy, win, 101 + temp->column * 75, 101 + temp->row * 25, 74, 24,
				   False);
			XFillRectangle(dpy, win, gc, 101 + temp->column * 75, 101 + temp->row * 25,
				       74, 24);
			size = strlen(display[temp->depth][temp->column][temp->row]);
			XSetForeground(dpy, gc, white);
			XSetBackground(dpy, gc, black);
			XDrawImageString(dpy, win, gc, 110 + 75 * temp->column,
					 117 + 25 * temp->row,
					 display[temp->depth][temp->column][temp->row], min(size,
											    7));
			XSetForeground(dpy, gc, black);
			XSetBackground(dpy, gc, white);
			temp = temp->next;
...
clear_demo_list()
{
	...
	while (demo_list != NULL) {
		XClearArea(dpy, win, 101 + demo_list->column * 75, 101 + demo_list->row * 25, 74,
			   24, False);
		size = strlen(display[demo_list->depth][demo_list->column][demo_list->row]);
		XDrawImageString(dpy, win, gc, 110 + 75 * demo_list->column,
				 117 + 25 * demo_list->row,
				 display[demo_list->depth][demo_list->column][demo_list->row],
				 min(size, 7));
...
run_demof()
{
	...
	while (run_demo) {
		found = 0;
		isis_sleep(delay_time);	/* allow for process to be in background until the end of
					   the delay time */
		demo_depth = get_spdsht_depth(dspdsht_list[0]);
		col = RANDOM() % 9;
		row = RANDOM() % 19;
		while (found == 0 && (time(&testtime) < (1 + delay_time + temp_time))) {
			...
			if (entry[demo_depth][col][row][0] == '=' ||
				...
				dig = 1;
				if (entry[demo_depth][col][row][1] == '-') {	/* i.e. a negative
										   number */
					dig = 2;
					tempstr[0] = '-';
				}
...
check_forward_dependencies(ldep, lcol, lrow)
	int ldep, lcol, lrow;		/* the depth, column and row we want to assign dependencies 
					   to */
{
	struct element *search;
	...
	while (search != NULL) {
		...
		if (test == 0) {
			...
			while ((ltemp->depth != ldep) || (ltemp->column != lcol)
			       || (ltemp->row != lrow)) {
				ltemp = ltemp->next;
				if (ltemp == NULL)
	...
	char locname[32];
{
	...
	while (spdsht_list[numb_spdshts][0] != '#' && spdsht_list[numb_spdshts][1] != '#') {
		if (numb_spdshts >= 10)
	...
	char spreadloc[32];
{
	...
	if (check == -1) {
		...
		/* strcpy (tempstr, "spread"); strcpy (tempgroup, strcat(tempstr, spreadloc)); */
		...
		while (spdsht_list[k][0] != '#' && spdsht_list[k][1] != '#') {
			if (strcmp(dspdsht_list[0], spdsht_list[k]) == 0 || number == 0) {
				...
				if (fp != NULL) {
					...
					sprintf(str, "%s %s", "unable to write to file",
						spdsht_list[k]);
					display_error(str, 2);
					/* printf("sorry couldn't write to file
					   %s\n",spdsht_list[k]); */
				}
			}	/* end if strcmp == 0 */
			k = k + 1;
		...
		case 2:	/* user has clicked on options menu */
			...
			case 1:
				if (run_demo == 0) {	/* was off turn it on */
					...
					display_error
					    ("Enter a single digit for the seconds delay you would like.",
					     1);

					while (!(XCheckTypedEvent(dpy, KeyPress, &event))) ;
					XClearArea(dpy, win, 200, 30, 800, 40, False);
					charcount =
					    XLookupString(&event.xkey, buffer, bufsize, &keysym,
							  &compose);
	...
	int x_coord;
{
	...
	while (choosing) {
		...
		case MotionNotify:
			XQueryPointer(dpy, menu_event.xmotion.window, &root, &child, &root_x,
				      &root_y, &pos_x, &pos_y, &key_but);
			if ((pos_x > (menu[menu_choice].left - menu_margin))
				...
				if (old_pane != sub_pane) {
					...
					XFillRectangle(dpy, win, gc,
						       menu[menu_choice].left - menu_margin + 2,
						       menu_bot + menu_space * sub_pane + 2,
						       menu[menu_choice].width + 2 * menu_margin -
						       2, menu_space - 2);
					draw_inverse_text(menu[menu_choice].sub_menu[sub_pane],
							  menu[menu_choice].left,
							  14 + menu_bot + menu_space * sub_pane);
					if (old_pane != -1) {	/* uninvert old sub menu assuming
								   there was one */
						XClearArea(dpy, win,
							   menu[menu_choice].left - menu_margin + 2,
							   menu_bot + menu_space * old_pane + 2,
							   menu[menu_choice].width +
							   2 * menu_margin - 2, menu_space - 2,
							   False);
						XDrawImageString(dpy, win, gc,
								 menu[menu_choice].left,
								 14 + menu_bot +
								 menu_space * old_pane,
								 menu[menu_choice].sub_menu
								 [old_pane],
								 strlen(menu[menu_choice].sub_menu
									[old_pane]));
					}
					old_pane = sub_pane;
				}
