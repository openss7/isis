...
test_point(s)
  ...
  /* Allow the user to force control particular event sequences, or failure modes in multiple
     processes. */
{
	if (test) {
		print("*** %s *** ", s);
...
complete(success)
  ...
  /* Commit or abort the transaction, returning TRUE if the commit was successful, and FALSE if the 
     transaction aborted. Optionally allow the user to do an abort. */
{
	bool commit;
	...
	int amount;
{
	...
	while ((repeat--) > 0 && success == 0) {
		...
		if (cbcast(server, BANK_DEPOSIT, "%s %d", name, amount, 1, "%d", &success) != 1) {
	...
	while ((repeat--) > 0 && success == 0) {
		...
		while (TRUE) {
			...
			if (cbcast(server, BANK_WITHDRAW,
				...
				if (success == bank_account_conflict && (++retries) < 3) {
					bank_error(success);
	...
	int *balance, *cleared;
{
	...
	while ((repeat--) > 0 && success == 0) {
		...
		if (cbcast(server, BANK_INQUIRY, "%s", name, 1, "%d %d", balance, cleared) != 1) {
			success = bank_service_unavailable;
			connected = FALSE;
		} else {
...
get_name(name, what)
  ...
  /* Obtain the next token using strtok, and parse it as a alphabetic name into 'name'. */
{
	*name = strtok(NULL, delim);
	if (*name == NULL || !alpha(*name)) {
		print("%s expected\n", what);
		return (FALSE);
	}
	return (TRUE);
...
get_number(num, what)
  ...
  /* Obtain the next token using strtok, and parse it as an integer into 'num'. */
{
	char *name;

	name = strtok(NULL, delim);
	if (name == NULL || !number(name)) {
		print("%s expected\n", what);
		return (FALSE);
	}
	*num = atoi(name);
...
bool
command()
  /* Read one line from input and execute it as a command. Return FALSE when there is no more
     input, or to quit. */
{
	char line[255];
	char *arg;
	...
	if (com_equal(arg, "deposit", 1)) {
		...
		if (get_name(&acc, "FROM account name") &&
		    get_name(&acc2, "TO account name") && get_number(&amount, "amount")) {
	...
	} else if (com_equal(arg, "help", 1) || com_equal(arg, "?", 1)) {
		help();
...
{
	print("Bank teller\n");
	/* Read in command line arguments: <port-nr> Isis port number (optional). */
