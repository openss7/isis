	...
	char *why;
{
	...
	if (level & DUMP_VIEWS) {
		...
		print("Site view %d/%d:", current_view.sv_viewid & 0xFF,
		      current_view.sv_viewid >> 8);
...
{
	if (addr_isgid(addr)) {
		print("(gid %d/%d.%d[%d])", addr->addr_site, addr->addr_incarn, addr->addr_groupid,
		      addr->addr_entry);
		return;
	}
	if (addr_isact(addr)) {
		print("(act %d/%d.%d[%d])", addr->addr_site, addr->addr_incarn, addr->addr_groupid,
		      addr->addr_entry);
		return;
	...
	if (tp->task_queue) {
		...
		if (tp->task_queue == &tp->task_mwant) {
			print("wants %d of %d replies, got %d+%d null, msgid=%d\n", tp->task_nwant,
			      tp->task_nsent, tp->task_nreplies, tp->task_nullreps, tp->task_msgid);
			print("        Dests: ");
...
dump_fdect()
{
	...
	print("\n  Pending recoveries:");
	for (qp = pending_recoveries->qu_next; qp != pending_recoveries; qp = qp->qu_next)
		print(" %d", qp->qu_name);
	print("\n  Replies wanted:");
	for (qp = replies_wanted->qu_next; qp != replies_wanted; qp = qp->qu_next)
		print(" %d/%d", SI(qp->qu_name));
	print("\n  View r_locks: ");
