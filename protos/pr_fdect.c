...
sview current_view;
int fd_coordinator, exists_proposal, exists_old_proposal, sent_old_proposal;
int proposed_viewid, old_proposed_viewid;
site_id proposed_slist[MAX_SITES + 1], old_proposed_slist[MAX_SITES + 1];
bitvec proposed_failed, proposed_recovered, old_proposed_failed, old_proposed_recovered;
...
#   endif FD_DEBUG
	...
	for (i = 0; i < (sizeof(current_view.sv_incarn) / sizeof(*current_view.sv_incarn)); i++)
...
#   endif FD_DEBUG
	...
	if (!fd_coordinator) {
		...
		msg = msg_newmsg();
		dest = ADDRESS(sender->addr_site, sender->addr_incarn, PROTOCOLS, FD_RECVNEGACK);
		msg_setdest(msg, &dest);
		pr_bcast(msg);
		msg_delete(msg);
	} else if (current_view.sv_incarn[sender->addr_site] != DOWN_INCARN)
	...
	if (!qu_find(pending_recoveries, (int) sender->addr_site)) {
		qu_add(pending_recoveries, sender->addr_site, (char *) 0, nullroutine);
		if (!fd_forked) {
			fd_forked = TRUE;
			fd_newview();
		}
...
#   endif FD_DEBUG
	...
	do {
		if (!exists_proposal) {
			proposed_viewid = INCR_VIEWID(current_view.sv_viewid);
			for (i = 0; i < (sizeof(proposed_slist) / sizeof(*proposed_slist)); i++)
		...
		if (qu_head(pending_failures)) {
			...
			do {
				...
				fd_msg = msg_newmsg();
				addr =
				    ADDRESS(SITE_NO(s_id), SITE_INCARN(s_id), PROTOCOLS,
					    FD_RECVINCARN);
		...
		if (fd_computechanges() == FD_TIMEOUT)
			...
			*dp = NULLADDRESS;
			if (dp != dests) {
				...
				msg_addfield(fd_msg, FLD_SLIST, (char *) proposed_slist,
					     FTYPE_SITEID, sizeof(proposed_slist));
				if (exists_old_proposal && !sent_old_proposal) {
					msg_addfield(fd_msg, FLD_OLDVIEWID,
						     (char *) &old_proposed_viewid, FTYPE_LONG,
						     sizeof(old_proposed_viewid));
					msg_addfield(fd_msg, FLD_OLDSLIST,
						     (char *) old_proposed_slist, FTYPE_SITEID,
						     sizeof(old_proposed_slist));
...
#           endif FD_DEBUG
		...
		if (participant_failed) {
			...
			if (fd_type == FD_GROW)
				while (qp = qu_head(recovering)) {
					register qnode *np =
					    qu_find(pending_recoveries, qp->qu_name);
					qu_free(np);
					qu_free(qp);
				}
		}
...
#   endif FD_DEBUG
	...
	if (view_id != current_view.sv_viewid) {
		...
		if (!kill_sender && exists_proposal && VIEW_NO(view_id) == VIEW_NO(proposed_viewid))
			for (s_id = slist; *s_id; s_id++)
				if (sender_id != proposed_slist[0] && *s_id == proposed_slist[0]) {
					kill_sender = TRUE;
					break;
				}
		...
		if (!refused) {
			...
			fd_type = *(int *) msg_getfield(fd_msg, FLD_FDTYPE, 1, (int *) 0);
			if (fd_type == FD_GROW) {
				bclr(&upsites);
			...
			if (old_idp = (int *) msg_getfield(fd_msg, FLD_OLDVIEWID, 1, (int *) 0)) {
				if (fd_type == FD_GROW)
...
fd_computechanges()
{
	...
	char up;
	site_id *old_slist, *new_slist, prop_id, old_id, deadlist[MAX_SITES];
...
#   endif FD_DEBUG
	...
	while (nloops-- > 0) {
		for (i = 0, up = TRUE; i < (sizeof(proposed_slist) / sizeof(*proposed_slist)); i++) {
			prop_id = new_slist[i];
			if ((prop_id == 0) && up)
				up = FALSE;
				...
				if (up && (SITE_INCARN(old_id) != SITE_INCARN(prop_id))) {
					if (SITE_NO(prop_id) == my_site_no) {
						if (SITE_INCARN(prop_id) != my_site_incarn)
							panic
							    ("fd_computechanges: I refuse to change %s",
...

{
	...
	if (*(int *) msg_getfield(fd_msg, FLD_VIEWID, 1, (int *) 0) != proposed_viewid)
		print("fd_recvack: late ack, current proposal %x\n", proposed_viewid);

#   ifdef FD_DEBUG
	print("fd_recvack: received ack for viewid %x ",
	      *(int *) msg_getfield(fd_msg, FLD_VIEWID, 1, (int *) 0));
	pmsg(fd_msg);
#   endif FD_DEBUG
...

{
	...
	for (dp = dests; *sp; sp++)
		if (*sp != my_site_id)
			*(dp++) = ADDRESS(SITE_NO(*sp), SITE_INCARN(*sp), PROTOCOLS, FD_RECVCOMMIT);
	*dp = NULLADDRESS;
	if (dp != dests) {
		fd_msg = msg_newmsg();
...
#   endif FD_DEBUG
	...
	if (exists_proposal)
		...
		panic
		    ("Site recovery occured too quickly after a failure! (Wait a while and try again)");
	msg_delete(fd_msg);
}
...
#       endif FD_DEBUG
		...
		if (fd_coordinator) {
			...
			if (btst(&current_view.sv_failed) || btst(&current_view.sv_recovered))
				...
				panic
				    ("fd_coordinator: 5 attempts to install new views have aborted!");
		}
			...
			for (s_id = failed_sites; *s_id; s_id++) {
				...
				for (ss_id = proposed_slist; *ss_id; ss_id++)
					if (SITE_NO(*s_id) == SITE_NO(*ss_id)) {
						if (!qu_find(pending_failures, *s_id))
							qu_add(pending_failures, *s_id, (char *) 0,
							       nullroutine);
		...
		if (recov_coordinator) {
			for (s_id = failed_sites; *s_id; s_id++)
				if (*s_id == recov_coordinator)
					panic
					    ("site restart: remote coordinator for my recovery not responding");
			return;
		}
		for (s_id = failed_sites; *s_id; s_id++)
...

{
	...
	for (i = 0; proposed_slist[i] && SITE_NO(proposed_slist[i]) != site_no; i++)
		continue;
...

{
	...
	do {
		...
		blocked = FALSE;
		for (node = pbufs->qu_next; !blocked && node != pbufs; node = node->qu_next) {
			proc = node->qu_name;
			qp = node->qu_queue;
	...
	if (w_struct.n_events > 0) {
		...
		if (rval == FD_TIMEOUT) {
			for (node = pb_itemlist->qu_next; node != pb_itemlist; node = node->qu_next) {
