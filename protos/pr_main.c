...
static struct timeval sel_wait = { 0, 250 }, no_wait = {
0, 0};
	...
	char **argv;
{
	...
	begin {
		...
		if (new == -1) {
			/* If the file already exists try to fix its protection or delete it. */
			...
			if (new == -1) {
				fprintf(stderr,
					"<isis-protos>: Unable to create system log file <%s>!\n",
					logfile);
				exit(-1);
			}
		}
...
scan_sfile()
{
	...
	if (warn_remote)
		print
		    ("Warning: 'sites' file gives multiple values for bcast port\n... (note that isis_remote uses this port number)\n");
	fclose(sfile);
	if (first_time++) {
		message *mp = msg_genmsg(CL_NEW_SNAMES, (char *) site_names, FTYPE_CHAR,
					 nsites * sizeof(*site_names), 0);
...
main_loop()
{
	...
	forever {
		...
		if (cl_congested == 0
		    && (memuse > MEM_HI || msgmem > MSGMEM_HI || ntasks > TASK_HI
			|| intersite_congested > INTERSITE_HI)) {
			++cl_congested;
			EVENT(S_CONGEST);
			cl_send_all(GENERIC_CONGESTED, (message *) 0);
			/* Trigger garbage collection */
			did_delete(1);
		} else if (cl_congested
			   && (memuse < MEM_LO && msgmem < MSGMEM_LO && ntasks < TASK_LO
			       && intersite_congested < INTERSITE_LO)) {
			--cl_congested;
		...
		if (btst(&in_mask) == 0) {
			...
			if ((rv =
			     select(max, &in_mask, &out_mask, &ex_mask,
				    mw ? &sel_wait : &no_wait)) == -1) {
				if (errno == EINTR)
					goto again;
				if (errno == EBADF) {
					register b;
					...
					for (b = 0; b < max; b++) {
						...
						if (bit(&congested, b))
							bis(&out_mask, b);
						if (select
						    (max, &in_mask, &out_mask, (int *) 0,
						     &no_wait) == -1 && errno == EBADF)
							if (b == net_socket)
			...
			if (mw) {
				...
				Gettime();
				if ((qp = qu_head(time_queue))
				    && (delta = (ISIS_TIME - qp->qu_time)) > 1000)
	...
	if (my_site_no) {
		if (running) {
			fprintf(stderr, "%s (%d/%d): -- panic --\n", my_host, my_site_no,
				my_site_incarn);
			fprintf(stderr, fmt, a0, a1, a2, a3, a4, a5, a6, a7, a8, a9, aa, ab, ac, ad,
				ae);
			fprintf(stderr, "\n");
		}
