...
pr_gbcast(msg)
	message *msg;
{
	int msg_id, n_dests, priority, *before, bef_size, ph1, *idp;
			...
			*priority = answ[i]->priority > *priority ? answ[i]->priority : *priority;
			for (j = 0; answ[i]->idlist[j] != 0; j++) {
				for (k = 0; k < n_before && (*before)[k] != answ[i]->idlist[j]; k++)
					continue;
				if (k == n_before)
					pr_assign((char *) &(*before)[n_before++],
						  (char *) &answ[i]->idlist[j], sizeof(**before),
						  (char **) before, bef_size);
			}
			free((char *) answ[i]);
			pr_assign((char *) &(*before)[n_before], (char *) 0, sizeof(*before),
				  (char **) before, bef_size);
		}
...

{
	...
	for (n_before = 0; before[n_before]; n_before++)
		continue;
	(void) msg_addfield(gb_msg, FLD_BEFORE, (char *) before, FTYPE_LONG,
			    (n_before + 1) * sizeof(*before));
#       ifdef GB_DEBUG
...
#       endif GB_DEBUG
	...
	for (sp = slist, dp = dests; *sp; sp++, dp++) {
		*dp = ADDRESS(SITE_NO(*sp), SITE_INCARN(*sp), PROTOCOLS, PR_GBRECVABORT);
		w_str.n_events++;
	}
	*dp = NULLADDRESS;
...
#           endif GB_DEBUG
		...
		if (local) {
			...
			if (coord->addr_site != my_site_no) {
				if (watch_id =
				    watch_on((int) coord->addr_site, (int) coord->addr_incarn,
					     gb_takeover, (char *) msg_id))
					(void) st_add(msg_id, QU_WATCH, (char *) watch_id,
						      watch_off);
				else
		...
		if (!local && ((answp = (gb_answer *) st_find_int(msg_id, QU_GBANSW)) != 0)
		    && (answp->priority == GB_ABORT)) {
#               ifdef GB_DEBUG
...
#           endif GB_DEBUG
		...
		for (pp = plist; *pp; pp++) {
			...
			do {
				...
				if (node = qu_find(idlists, *pp)) {
					...
					for (id_node = idlist->qu_next;
					     !blocked && id_node != idlist; id_node = id_next) {
						id_next = id_node->qu_next;

						id = id_node->qu_name;
						if (!qu_find(done_id, id)) {
							for (i = 0;
							     i < n_before && answp->idlist[i] != id;
							     i++)
								continue;
							if (i == n_before) {
								qnode *npb_node;

								for (pb_node = pbufs->qu_next, prc =
								     pb_node->qu_name;
		...
		if (!local) {
			while (w1_qp->qu_cond)
				t_sig(&w1_qp->qu_cond, 0);
			qu_free(w1_qp);
			if (((answpp = (gb_answer *) st_find_int(msg_id, QU_GBANSW)) != 0)
			    && (answpp->priority == GB_ABORT)) {
#                   ifdef GB_DEBUG
...
#       endif GB_DEBUG
	...
	else if (st_deleteable(msg_id) || !(phase = st_find_int(msg_id, QU_PHASE)) || phase == 2) {
	...
	if (!local) {
		...
		pr_assign((char *) &answp->idlist[0], (char *) before, before_len, (char **) &answp,
			  &ans_len);
			...
			if (w_node = qu_find(wait_queues, pid)) {
				...
				for (node = waitq->qu_next; node != waitq; node = next) {
					...
					if (node->qu_witem.deleted
					    || (died
						&& addr_cmp(msg_getsender(node->qu_witem.msg),
							    died) == 0))
						j = -1;
					else {
...
#                       endif GB_DEBUG
	...
	if (local) {
		...
		if (coord->addr_site != my_site_no) {
			if (watch_id =
			    watch_on((int) coord->addr_site, (int) coord->addr_incarn, gb_takeover,
				     (char *) msg_id))
			...
			if (qp = qu_find(idlists, *proc)) {
				...
				for (id_node = idlist->qu_next; !blocked && id_node != idlist;
					...
					if (!st_deleteable(id) && !qu_find(done_id, id)) {
						for (pb_node = pbufs->qu_next, prc =
						     pb_node->qu_name; !blocked && pb_node != pbufs;
						     pb_node = pb_node->qu_next)
							blocked = cb_sendpkt(prc, id, &w_struct);
						if (!blocked) {
							pr_assign((char *)
								  &answp->idlist[n_before++],
								  (char *) &id, sizeof(id),
								  (char **) &answp, &ans_size);
							qu_add(done_id, id, (char *) 0,
							       nullroutine);
						}
					}
				}
...
#       endif GB_DEBUG
	...
	pr_allocinit((char **) &before, &bef_size);
	if ((ph1 = gb_send1(msg_id, msg, slist, &priority, &before, &bef_size)) == GB_ABORT) {
		gb_abort(msg, msg_id);
		gb_sendabort(msg_id, slist);
		free((char *) before);
		gb_free(msg_id);
