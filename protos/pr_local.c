	...
	if (pp = plist) {
		...
		for (ap = alist; !aptr_isnull(ap); ap++)
			if (ap->addr_site == my_site_no
			    && (ap->addr_incarn == RECOVERY_INCARN
				|| ap->addr_incarn == my_site_incarn
				|| my_site_incarn == RECOVERY_INCARN)
			    && ap->addr_process != my_process_id) {
				for (bp = alist; bp != ap; bp++)
					if (bp->addr_process == ap->addr_process
					    && bp->addr_site == ap->addr_site)
						break;
				if (bp == ap) {
					++cnt;
