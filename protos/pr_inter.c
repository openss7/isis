...
#define TIME_FAIL      FDTIMEOUT/2	/* Detect failures after, say, 10 times the rtdelay, which
					   varies */
...
net_init()
{
	...
	if (bind(net_socket, (struct sockaddr *) &my_addr, sizeof(my_addr)) == -1) {
		perror("bind");
		panic("<protos> was unable to acquire UDP port %d.  Was ISIS run twice?",
...
# endif SCATTERSEND
	{
		...
		while (iovlen--) {
	      ...
	      loop:
		if (sendto
		    (net_socket, staging_buf, len, 0, (struct sockaddr *) &ioqp->io_address,
		     sizeof(saddr)) != -1)
			errno = 0;
...
net_drain_any_input()
{
	...
	while (tank_status() != TANK_FULL
	       && select(MAXBITS, &tank_mask, (int *) 0, (int *) 0, &tank_wait) == 1) {
		Gettime();
...
net_drain_one_input(flag)
{
	...
	net_xmits = 0;
	fromlen = sizeof(from);
	nb = recvfrom(net_socket, &len, sizeof(long), MSG_PEEK, (struct sockaddr *) &from,
		      &fromlen);
	if (nb != sizeof(long))
		return;
	len = ntohl(len);
...
net_deliver()
{
	...
	if (SITE_INCARN(is->is_from) != RECOVERY_INCARN && my_site_incarn != RECOVERY_INCARN) {
		...
		if (is->is_dest != my_site_id) {
			...
			if (ioqp->io_dead && ioqp->io_dead == is->is_from) {
				...
				print("DISCARD: %x expected %x RECOVERY_INCARN %d\n", is->is_from,
				      ioqp->io_sid, RECOVERY_INCARN);
			goto discard;
		}
	} else if (SITE_INCARN(is->is_from) == RECOVERY_INCARN) {
		if (my_site_incarn == RECOVERY_INCARN || is->is_dest != my_site_id
		    || fd_coordinator == 0) {
			if (my_site_incarn == RECOVERY_INCARN)
		...
		if ((ioqp->io_state & IO_ESTAB)
		    || (is->is_seqn == 1 && SITE_INCARN(ioqp->io_sid) != RECOVERY_INCARN)) {
			if ((ioqp->io_state & (IO_ALIVE | IO_TOLDFD)) == IO_ALIVE) {
				print
				    ("Site %d/%d seems to have died (unexpectedly saw RECOVERY request)\n",
				     SITE_NO(ioqp->io_sid), SITE_INCARN(ioqp->io_sid));
	...
	if (is->is_seqn != SEQ_ACK && in_net_input == 1) {
		...
		while (*(in = &ioqp->io_in[INC(ioqp->io_iseqn, 1) & MAXWMASK])) {
			...
			register m;
			register intersite *iis =
			    (intersite *) msg_getfield(*in, INTERSITE_HDR, 1, (int *) 0);
			if (iis->is_viewid != current_view.sv_viewid
			    && iis->is_viewid != strange_id)
...
#                       ifdef INTERVERBOSE
					print("*** NET: TRY TO REASSEMBLE ");
					pmsg(msgs[n]);
#                       endif INTERVERBOSE
					fragman(msgs[n]);
				} else {
#                       ifdef INTERVERBOSE
					print("*** NET: TRY TO DELIVER ");
					pmsg(msgs[n]);
					msg_printaccess(msgs[n]);
#                       endif INTERVERBOSE
					EVENT(S_IMSGS);
	...
	register intersite *is;
{
	...
	if (is->is_abits && ioqp->io_oseqn != ioqp->io_nseqn
	...
	while (ioqp->io_oseqn != ioqp->io_nseqn
		...
		if (out->pk_nret == 0) {
			...
			if ((dt = now - out->pk_whensent + 1 * sec) <= 2 * sec)
				dt = 3 * sec;
			if ((ioqp->io_rtdelay =
			     ((ioqp->io_rtdelay << 3) - ioqp->io_rtdelay + dt) >> 3) < MIN_RET)
...
net_sweep()
{
	...
	for (qp = IOQS->qu_next; qp != IOQS; qp = nqp) {
		...
		nqp = qp->qu_next;
		if ((ioqp->io_state & (IO_ALIVE | IO_TOLDFD)) == IO_ALIVE) {
			if ((ioqp->io_sid == on_my_right
			     || bit(&proposed_recovered, SITE_NO(ioqp->io_sid)))
			    && --hello_delay <= 0) {
				...
				if ((out->pk_state & (PK_INUSE | PK_ACKED)) == PK_INUSE) {
					...
					if (now > out->pk_rtwhen) {
						if (out->pk_nret == MAX_RET) {
							print
							    ("Transmitted same packet %d times, giving up (len %d)\n",
		...
		if ((ioqp->io_state & (IO_ESTAB | IO_ALIVE | IO_TOLDFD)) == (IO_ESTAB | IO_ALIVE)) {
			...
			/* Detect failures */
			if (ioqp->io_nslots < WSIZE && t >= 30 * sec
			    && t >= (ioqp->io_rtdelay * TIME_FAIL)) {
		...
		if ((ioqp->io_state & (IO_ESTAB | IO_ALIVE | IO_TOLDFD)) == IO_ALIVE) {
			...
			if (ioqp->io_nslots < WSIZE && now - ioqp->io_lastin >= TIME_ESTAB) {
				print
				    ("Timeout during restart: connection not established after %d secs\n",
				     (now - ioqp->io_lastin) / sec);
			      failed:
				print("Site %d/%d seems to have died\n", SITE_NO(ioqp->io_sid),
				      SITE_INCARN(ioqp->io_sid));
				ioqp->io_state |= IO_TOLDFD;
				*sp++ = ioqp->io_sid;
			}
		}
	...
	register ioq *ioqp;
{
	...
	if (ioqp->io_state) {
		print("  %d/%d\t[%s]:\n\t", SITE_NO(ioqp->io_sid), SITE_INCARN(ioqp->io_sid),
		      site_names[SITE_NO(ioqp->io_sid)]);
		if (ioqp->io_state) {
			if (ioqp->io_state & IO_ESTAB)
				print("estab;");
			if (ioqp->io_state & IO_ALIVE)
		...
		for (p = ioqp->io_oseqn; p != ioqp->io_nseqn; p = INC(p, 1)) {
			...
			print("\t... packet %d = sl%d,sz%d,cnt%d", p, out - ioqp->io_out,
			...
			if ((out->pk_state & PK_ACKED) == 0) {
				...
				whs = ws / 10 % 100;
				ws /= 1 * sec;
				print("next ret in %d.%.2d", out->pk_nret, ws, whs,
				      out->pk_rtwhen - now);
			}
			print(", state=<");
			if (ioqp->io_accum == out)
		...
		while (p != INC(ioqp->io_oseqn, WSIZE)) {
			...
			if (out->pk_packet && out->pk_state == PK_INUSE && p == ioqp->io_nseqn)
				...
				if (out->pk_packet)
					print
					    ("\t*** FOUND A PACKET IN SEQN %d... oseqn %d nseqn %d!\n",
					     p, ioqp->io_oseqn, ioqp->io_nseqn);
				if (out->pk_state)
					print
					    ("\t*** FOUND STATE %x in SEQN %d!... oseqn %d nseqn %d\n",
