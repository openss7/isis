		...
		while (nresp < nsent && nleft > 0 && (qp = qu_head(ctp->task_msgs))) {
			...
			for (ap = alist; !aptr_isnull(ap); ap++)
				if (addr_cmp(ap, &from) == 0) {
					...
					if (abtype =
					    (int *) msg_getfield(rmsg, FLD_ISABORTREP, 1,
								 (int *) 0)) {
						if (collect)
							(*collect) (UNCOLLECT, rmsg, answ, alen,
								    nresp);
						nleft = 0;
						nresp = *abtype;
						break;
					...
					if (done[al] != 'W' && done[al] != '*')
						...
						/* Forwarded message */
						if (SITE_IS_UP
						    (fwd[FWI_NEWDEST].addr_site,
						     fwd[FWI_NEWDEST].addr_incarn)) {
							bis(&ctp->task_watching,
							    fwd[FWI_NEWDEST].addr_site);
							*ap = fwd[FWI_NEWDEST];
							done[al] = '*';
							break;
		...
		for (tqp = tasks->qu_next; tqp != tasks; tqp = ntqp) {
			...
			if (bitv(&tqp->qu_task->task_watching, &current_view.sv_failed)) {
				...
				for (s = 0; s < MAX_SITES; s++)
					if (bit(&current_view.sv_failed, s)
					    && bit(&tqp->qu_task->task_watching, s)) {
						qu_add(tqp->qu_task->task_msgs, MC_ISSNO, s,
						       nullroutine);
	...
	register message *msg;
{
	...
	register sys_groupview *pg =
	    (sys_groupview *) msg_getfield(msg, SYSFLD_TRUEVIEW, 1, (int *) 0);
	register qnode *qp;
