...
#       endif
	...
	msg_insertref(msg, FLD_ANSW, (char *) &current_view, FTYPE_SVIEW, sizeof(current_view),
		      (vfunc *) nullroutine);
	msg_insertref(msg, FLD_ANSW, (char *) site_names, FTYPE_CHAR, (nsites + 1) * 64,
		      (vfunc *) nullroutine);
...
pr_local_rcv(sd)
{
	...
	if (mp = msg_reconstruct(cp->cl_blkdesc)) {
		EVENT(S_SYSCALLS);
#	    if (CL_DEBUG)
		print("rcv from client %d: ", cp->cl_pid);
		pmsg(mp);
#	    endif
		if (cp->cl_pid > 0) {
			address sender;
...
#	if (CL_DEBUG)
	print("send to client %d: ", cp->cl_pid);
	pmsg(mp);
#	endif
	cp->cl_curmsg = mp;
	...
	for (sd = 0; sd <= max_cl; sd++)
		if (bit(&sdlist, sd)) {
			...
			if (nb == -1 && errno != EWOULDBLOCK)
				...
				cl_xfered(cp, nb);
				while (cp->cl_pid && (cp->cl_flag & CLIENT_BUSY) == 0
				       && cp->cl_queuelen) {
	...
	address *pname;
{
	...
	for (cp = cl; cp <= &cl[max_cl]; cp++)
		if ((cp->cl_flag & CLIENT_PMONITOR) && (qp = pg_find(cp->cl_pmonitor, pname))) {
			...
			register message *mp =
			    msg_genmsg(CL_PNAME, (char *) pname, FTYPE_ADDRESS, sizeof(address), 0);
			qu_free(qp);
			addrs[0] = cp->cl_paddr;
...
client_newview()
{
	...
	for (cp = cl; cp <= &cl[max_cl]; cp++) {
		address addr;
		register message *mp =
		    msg_genmsg(CL_VIEW, (char *) &current_view, FTYPE_CHAR, sizeof(sview), 0);
		addr = cp->cl_paddr;
		...
		if (cp->cl_flag & CLIENT_PMONITOR)
			for (qp = cp->cl_pmonitor->qu_next; qp != cp->cl_pmonitor; qp = nqp)
				if (current_view.sv_incarn[qp->qu_pname.addr_site] !=
				    qp->qu_pname.addr_incarn) {
	...
	register failinfo *fip;
{
	...
	for (s = 1; s < MAX_SITES; s++)
		if ((s == my_site_no || bit(&fip->fi_watchedby, s))
		    && (i = current_view.sv_incarn[s]) >= 0)
			*sp++ = MAKE_SITE_ID(s, i);
	*sp = 0;
...
dump_clients()
{
	...
	for (sd = 0; sd <= max_cl; sd++) {
		...
		if (cp->cl_pid) {
			...
			print(": ");
			if (cp->cl_flag & CLIENT_BUSY)
				print("busy (%d bytes in transit, %d bytes queued) ", cp->cl_nbytes,
				      cp->cl_queuelen);
			else
				print("idle");
			...
			if (cp->cl_flag & CLIENT_PMONITOR) {
				...
				print(" monitoring < ");
				for (qp = cp->cl_pmonitor->qu_next; qp != cp->cl_pmonitor;
				     qp = qp->qu_next) {
					paddr(&qp->qu_pname);
					print(" ");
				}
			...
			if (btst(&cp->cl_watchedby)) {
				...
				for (s = 1; s < MAX_SITES; s++)
					if (bit(&cp->cl_watchedby, s)
					    && current_view.sv_incarn[s] != DOWN_INCARN)
						print("%d ", s);
				print(">");
			}
			...
			if (cp->cl_msgqueue == 0)
				...
				for (qp = cp->cl_msgqueue->qu_next; qp != cp->cl_msgqueue;
				     qp = qp->qu_next) {
