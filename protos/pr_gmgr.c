	...
	if (viewid) {
		...
		if (pg && pg->pg_viewid == *viewid) {
			if (dont_reply == 0 && (msg_getid(mp) & 1) == 0)
				REPLY(&from, &msg_getid(mp), &dest, mp, (char *) -1, 0, 0, 0, 0, 0,
				      0);
			return (0);
		}
		if (dont_reply == 0) {
			if (from.addr_site == my_site_no)
				pg = 0;
			REPLY(&from, &msg_getid(mp), &dest, mp, (char *) -1, IE_AGAIN, 0,
			      SYSFLD_TRUEVIEW, (char *) pg, FTYPE_PGROUP, pg ? pglength(pg) : 0);
		}
		return (-1);
	}
	return (0);
		...
		if (bit(&(*vi)->vi_sites, my_site_no)) {
			...
			for (ap = dests; !aptr_isnull(ap); ap++)
				if (ap->addr_site == my_site_no) {
					...
					++fnd;
#                       if (GB_DEBUG)
					print("... pg_verify_view: found viewid %d of ",
					      qp->qu_pg->pg_viewid);
					paddr(&qp->qu_pname);
					print("\n");
#                       endif
	...
	register message *mp;
{
	...
	forever {
		...
		if (pg_creating <= CR_NULL
		    || (pg_creating == CR_CHECKING && sender.addr_site <= my_site_no)) {
	...
	int pid;
{
	...
	for (qp = cp->qu_next; qp != cp; qp = nqp) {
		...
		if (pg->pg_flag & PG_CACHED) {
			...
			msg_insertfield(mp, CL_GID, (char *) &pg->pg_gid, FTYPE_ADDRESS,
					sizeof(address));
...
pg_update_pgviews()
{
	...
	for (cl_root = pg_root->qu_next; cl_root != pg_root; cl_root = cl_root->qu_next)
		...
		for (qp = cl_root->qu_queue->qu_next; qp != cl_root->qu_queue; qp = qp->qu_next) {
			...
			for (sp = pg->pg_alist;
			     sp != &pg->pg_alist[pg->pg_nmemb + pg->pg_nclient + 1]; sp++)
			...
			for (ap = pg->pg_alist;
			     ap != &pg->pg_alist[pg->pg_nmemb + pg->pg_nclient + 1]; ap++)
				if (ap->addr_site
				    && SITE_IS_UP(ap->addr_site, ap->addr_incarn) == 0) {
					died[0] = *ap;
					addrs[0] = pg->pg_gid;
					addrs[0].addr_entry = GENERIC_DELETE;
					mp = msg_genmsg(CL_PNAME, (char *) died, FTYPE_ADDRESS,
							sizeof(address) * 2, 0);
					msg_insertfield(mp, CL_GID, (char *) &pg->pg_gid,
							FTYPE_ADDRESS, sizeof(address));
					msg_setsender(mp, sp);
					msg_setreplyto(mp, &my_address);
					msg_insertfield(mp, SYSFLD_VCHANGE, (char *) 0, FTYPE_CHAR,
							0);
					GBCAST(addrs, mp, ALL, collect_answ, answ, 1);
					msg_delete(mp);
					goto again;
	...
	while (n < ADDR_LEN && !aptr_isnull(&alist[n])) {
		if (died[n]) {
			...
			if ((cl_root = pg_find(pg_root, &client)) == 0)
				return;
			for (ap = cl_root->qu_queue->qu_next; ap != cl_root->qu_queue;
			     ap = ap->qu_next) {
				register sys_groupview *pg;
				register address *mp;
		...
		if (gp)
			for (ap = gp->qu_pg->pg_alist; !aptr_isnull(ap); ap++)
				if (addr_cmp(ap, &addr) == 0) {
					register signo =
					    *(int *) msg_getfield(mp, CL_SIGNO, 1, (int *) 0);
		...
		if ((pg->pg_flag & PG_CACHED) && pgcont) {
			...
			case GENERIC_DELETE:
				if (memb->addr_site != 0xFF) {
					for (ap = pg->pg_alist;
					     ap != &pg->pg_alist[pg->pg_nmemb + pg->pg_nclient];
					     ap++) {
						register address *dp;
...
#		if (PG_DEBUG)
				...
				paddr(memb);
				print("msg_id = %x, ",
				      *(int *) msg_getfield(mp, SYSFLD_PROTID, 1, (int *) 0));
				pmsg(mp);
#               endif
		...
		if (addr_cmp(&pname, memb) && entry != GENERIC_DELETE) {
			...
			print("req %s ", entry == GENERIC_ADDMEMB ? "add" : "del");
			paddr(memb);
			print("\n");
			pmsg(mp);
	...
	if (cl_known != -1) {
		...
		if (entry != GENERIC_DELETE && SITE_IS_UP(memb->addr_site, memb->addr_incarn) == 0
		    && update_is_running++ == 0)
	...
	message *msg;
{
	...
	if ((cl_root = pg_find(pg_root, &sender)) == 0)
		cl_root = pg_add_qu(pg_root, &sender, qu_null());
	if ((cp = pg_find(cl_root->qu_queue, &gid)) == 0
	    && (cp = cview_refresh(sender, cl_root, gid, 0)) == 0)
		return ((sys_groupview *) 0);
	...
	if (pg->pg_flag & PG_CACHED) {
		msg_replacefield(msg, SYSFLD_VIEWID, (char *) &pg->pg_viewid, FTYPE_LONG,
				 sizeof(int));
		msg_replacefield(msg, SYSFLD_GID, (char *) &gid, FTYPE_ADDRESS, sizeof(address));
		++pg->pg_ccount;
	} else if (pg->pg_ccount)
	...
	address sender, gid;
{
	...
	for (cqp = pg_root->qu_next; cqp != pg_root; cqp = cqp->qu_next)
		if ((qp = pg_find(cqp->qu_queue, &gid))
		    && ((pg = qp->qu_pg)->pg_flag & PG_CACHED) == 0) {
			...
			for (qp = cqp->qu_next; qp != cqp; qp = nqp) {
				nqp = qp->qu_next;
				if (qp != cp && strcmp(qp->qu_pg->pg_name, pg->pg_name) == 0
				    && qp->qu_pg->pg_ccount == 0)
					qu_free(qp);
			}
			return (cp);
		}
	...
	if (cp) {
		...
		while (n--) {
			...
			if ((count =
			     BCAST_SL(slist, PROTOCOLS, PR_PGWANTVIEW, msg, 1, collect_answ,
				      (char *) pg, sizeof(sys_groupview))) == 1)
				break;
		}
	}
	if (count == 0)
		...
		count =
		    BCAST_V(&current_view, PROTOCOLS, PR_PGWANTVIEW, msg, 1, collect_answ,
			    (char *) pg, sizeof(sys_groupview));
	msg_delete(msg);
	if ((cl_root = pg_find(pg_root, &sender)) == 0
	    || check_proc_status(sender.addr_process, 0) == 0) {
		pg_free(pg);
		return (0);
	}
